...
\end{equation}
with $\lambda_{ps}=V_{pb}^{} V_{p s}^\ast$. The $O_{1,2}^p$,  $O_{3,\ldots,6}$, and $O_{8}$ are the so-called ``current-current'', ``QCD-penguins'' and ``chromo-magnetic'' operators, respectively, and they contribute to the $b\to s\gamma$ and $b\to s\ell\ell$ transitions via an electromagnetic interaction. Finally, the $O_{7}$ and $O_{9,10}$ are the "electromagnetic dipole" and the "semileptonic operators", respectively.
