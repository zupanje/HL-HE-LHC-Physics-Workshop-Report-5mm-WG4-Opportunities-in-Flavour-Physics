...
\subsubsubsection{$B_{s,d}\to\ell\ell^\prime$}

From the theory perspective, the the purely leptonic decays, $B_s\to \ell^+ \ell'^-$ are the cleanest exclusive decays. Up to QED corrections~\cite{1708.09152}, all QCD effects are contained in a decay constant  which is precisely and reliably computed in the lattice, $f_{B_s}=228.4(3.7)$~MeV (see Sec.~11).
...
Future measurements at the LHC, including the upgrade, will bring this experimental uncertainty down to the percent level (see Sec.~\ref{}). Beyond the SM, the decays $B_{d,s}\to \mu^+ \mu^-$ give very strong constraints to the scalar and pseudoscalar operators, and also to $C^\mu_{10}$, which has an impact on the fits and the $b\to s\mu\mu$ anomalies.
