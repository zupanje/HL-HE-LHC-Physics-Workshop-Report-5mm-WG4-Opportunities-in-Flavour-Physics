...
In this section we discuss the status of the interpretation of $b\to s$ transitions within the Standard Mo and the prospects from future sensitivities at the LHC, and its complementarity with
the corresponding measurements at Belle~II. All current and planned measurements of processes involving $b\to s$ transitions are related to weak decays of $b$-hadrons. At present, most of the data is on $B$ decays, with a small fraction of $B_s$ decays. LHCb has already provided a few measurements on $\Lambda_b$ decays, with data sets that will increase dramatically in future runs, including a significant output on decays of other $b$-hadrons such as $\Omega_b$
or $\Xi_b$~\cite{EoI}. Belle~II will provide a wealth of data on $B$ decays until 2025~\cite{Belle2}, and ATLAS and CMS will continue to contribute to key channels
such as $B_{d,s}\to \mu^+\mu^-$ and $B\to K^*\mu^+\mu^-$. But the bulk of the $b$-hadron decay program in the next 20 years will be provided by the LHCb
experiment. The LHCb Phase-II upgrade is planned for Run-5 and will allow the detector to benefit fully from the HL upgrade of the LHC, reaching
the milestone of $300$~fb$^{-1}$. Considering the tremendous advancements in our knowledge of $b\to s$ transitions achieved with the $3$~fb$^{-1}$ of
Run 1~\cite{Altmannshofer:2014rta,1510.04239}, it is clear that the HL phase will change our paradigms.
...
operators with Wilson coefficients (WCs) that encapsulate all SM weak and heavy NP effects. Thus all observables can be calculated in full generality (model-independently) in terms of these WCs and of QCD hadronic matrix elements. In the case of $b\to s$ transitions, the relevant set of effective operators 
includes the electromagnetic and chromomagnetic operators ${\cal O}^{(\prime)}_{7\gamma}$ and ${\cal O}^{(\prime)}_{8g}$,
the semileptonic operators ${\cal O}^{(\prime)\ell}_{9,10,S,P,T}$ and various 4-quark operators, including ${\cal O}_{1c,2c}$
which have large Wilson coefficients. See e.g.~\cite{0811.1214,1704.06639} for definitions and conventions.
For the purpose of this Section, the piece of the effective hamiltonian relevant for the interpretation of the $b\to s\ell\ell$ data in terms of New Physics is
%
\begin{equation}
\label{eq:Heff}
\mathcal{H}_\text{eff}^\text{NP} = - \frac{4\,G_F}{\sqrt{2}} V_{tb}V_{ts}^* \frac{e^2}{16\pi^2}
\sum_{i, \ell}
(C_i^\ell O_i^\ell + C_i^{\prime\,\ell} O_i^{\prime\,\ell}) + \text{h.c.} \,,
\end{equation}
%
where the relevant semi-leptonic operators read
%
\begin{align}
O_9^\ell &=
(\bar{s} \gamma_{\mu} P_{L} b)(\bar{\ell} \gamma^\mu \ell)\,,
& \!\!\!\!\!
O_9^{\prime \, \ell} &=
(\bar{s} \gamma_{\mu} P_{R} b)(\bar{\ell} \gamma^\mu \ell)\,,\label{eq:O9}
\\
O_{10}^\ell &=
(\bar{s} \gamma_{\mu} P_{L} b)( \bar{\ell} \gamma^\mu \gamma_5 \ell)\,,
& \!\!\!\!\!
O_{10}^{\prime \, \ell} &=
(\bar{s} \gamma_{\mu} P_{R} b)( \bar{\ell} \gamma^\mu \gamma_5 \ell)\,,\label{eq:O10}
\end{align}
%
with $\ell=e,\mu$.
The semi-leptonic operators with scalar or tensor structure cannot explain the observed deviations, nor can the 4-quark operators if there is lepton-flavor
non-universality (however, see~\cite{Jager:2017gal}).
...
radiative, leptonic and semileptonic inclusive and exclusive decays of $b$-hadrons. The inclusive observables used in current fits are the branching fractions
...
an interesting effect in the fits, regarding the current anomalies and $C^\mu_{9}$~\cite{Belle2}. In particular, Belle-II measurements of $B\to X_s \mu^+\mu^-$ will be able
to confirm the LHCb anomalies by themselves by 2024 if they are there~\cite{Belle2}. Theoretically, these inclusive rate are dominated by the perturbative
...
On the exclusive side, the cleanest from the theory perspective are the purely leptonic decays $B_s\to \ell^+ \ell'^-$. Up to QED corrections~\cite{1708.09152}, all QCD effects are contained in the decay constant $f_{B_s}=224(5)$~MeV~\cite{FLAG}, which is precisely and reliably computed in the lattice.
The current theoretical uncertainty on the muonic branching ratio is at the level of 5\%~\cite{1708.09152}, while the latest experimental LHCb measurement has an error of~$\sim 25\%$~\cite{Aaij:2017vad}.
Future measurements at the LHC, including the upgrade, will bring this experimental uncertainty down to $\sim 5\%$ by the end of Run~4, and
$\sim 2\%$ by then end of Run-5 (extrapolating naively for a six-fold increase in luminosity). While $B_s\to \mu^+ \mu^-$ is not sensitive to $C^\mu_{9}$, it does constrain
very strongly the scalar and pseudoscalar operators, and indirectly also $C^\mu_{10}$, which has an impact on the fits and the $b\to s\mu\mu$ anomalies.
