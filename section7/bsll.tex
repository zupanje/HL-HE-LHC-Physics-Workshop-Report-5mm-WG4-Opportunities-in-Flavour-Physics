...
operators with Wilson coefficients (WCs) that encapsulate all SM weak and heavy NP effects. Thus all observables can be calculated in full generality (model-independently) in terms of these WCs and of QCD hadronic matrix elements. The relevant effective Lagrangian for $b\to s\ell\ell$ transitions at low-energies in the SM is
\begin{equation}
\label{eq:Heff}
  {\cal{H}}_{\rm eff}^{\rm SM}=
  \frac{4 G_F}{\sqrt{2}}\sum_{p=u,c}\lambda_{ps}\left(C_1 \mathcal{O}_1^p+C_2 O_2^p +\sum_{i=3}^{10} C_i  O_i\right),
\end{equation}
with $\lambda_{ps}=V_{pb}^{} V_{p s}^\ast$. The $O_{1,2}^p$,  $O_{3,\ldots,6}$, and $O_{8}$ are the so-called ``current-current'', ``QCD-penguins'' and ``chromo-magnetic'' operators, respectively, and they contribute to the $b\to s\ell\ell$ transition via an electromagnetic interaction. The $O_{7}$ and $O_{9,10}$ are the electromagnetic penguin and the semileptonic operators, respectively.
The effect of BSM enters through the WCs $C_1,\ldots,C_{10}$ or by means of chirally-flipped ($b_{L(R)}\to b_{R(L)}$) versions of these operators $O_{7,\ldots, 10}'$, and also four scalar and two tensor operators~\cite{Bobeth:2007dw}. Furthermore, in BSM, semileptonic operators can induce lepton-flavor universality violation (LFUV) or charged-lepton flavor violation (CLFV).

For the purpose of this Section, the relevant operators for the interpretation of the $b\to s\ell\ell$ data in terms of short-distances are
%
\begin{align}
&O_{7\gamma}^{(\prime)}=\frac{e}{16\pi^2}(\bar s\sigma_{\mu\nu}P_{R(L)} b)F^{\mu\nu},\\
&O_9^{\ell(\prime)} =\frac{\alpha_{\rm em}}{4\pi}
(\bar{s} \gamma_{\mu} P_{L(R)} b)(\bar{\ell} \gamma^\mu \ell)\,,
\,\,
O_{10}^{\ell(\prime)} =\frac{\alpha_{\rm em}}{4\pi}
(\bar{s} \gamma_{\mu} P_{L(R)} b)( \bar{\ell} \gamma^\mu \gamma_5 \ell),\label{eq:O10}
\end{align}
with $\ell=e,\mu$.
