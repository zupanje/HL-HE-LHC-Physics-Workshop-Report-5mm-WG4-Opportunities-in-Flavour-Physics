...
of the trigger efficiency as a function of the $\mathrm{B^0_s}$ decay time. This uncertainty is expected to be well measured 
 with the $\mathrm{B^+} \to J/\psi K^+$ decay using similar triggers. 
An unbinned maximum likelihood fit to the dimuon invariant mass distribution is performed by CMS to determine the $\mathrm{B^0_s} \to \mu ^+ \mu^-$ effective 
...
Fig~\ref{fig:CMS_lifetime} shows the $\mathrm{B^0_s}$ decay time distribution with the fit projection superimposed, for a simulated statistics comparable
...
bins of \qsq or using a \qsq-unbinned approach~\cite{Hurth:2017sqw}; LHCb foresees to achieve around 440\,000 fully reconstructed \decay{\Bz}{\Kstarz\mumu} decays, CMS \cite{CMS-PAS-FTR-18-033} around 700\,000 excluding the $q^2$ range 
 ...
 For LHCb the measurement is currently heavily statistically limited, and the major systematics are those which affect the calculation of the angular acceptance. Most of the systematic uncertainties are expected to reduce significantly with more integrated luminosity due to larger control samples. Both the LHCb statistical and systematics uncertainties are therefore scaled with integrated luminosity when obtaining the projections. In this context it is interesting to note that \upgradetwo will provide signal yields that are of the order of the current tree-level control modes $B^0\to \jpsi \Kstarz$ and $B^0_s\to \jpsi\phi$. The most recent determination of $\phis$ via a time-dependent angular analysis of $B^0_s\to \jpsi\phi$ the current systematic uncertainties on the decay amplitudes due to modeling of the angular acceptance are already at the 0.001–0.002 level which would be sufficient to not systematically limit the \upgradetwo analysis.
...
Uncertainties concerning the simulation mismodeling, the efficiency determination,
 ...
 For both the Run~3 and HL-LHC datasets,the precision of the measurement of $b\to sll$ branching fractions will be limited by the knowledge of the \decay{\B}{\jpsi X} decay modes that are used to normalise the observed signals. 
...
Scenarios~I and II are inspired by the current discrepancies. The first scenario is the one that best explains the present rare semileptonic decay data. The second scenario best explains the rare semileptonic measurements if a purely left-handed coupling to quarks and leptons is required for NP. This requirement is theoretically well motivated and arises in models designed to simultaneously explain the discrepancies seen in both tree-level semitauonic and loop-level semileptonic decays. The third and fourth scenarios assume that the current discrepancies are not confirmed but there is instead a small contribution from right-handed currents that would not be visible with the current level of experimental precision. These scenarios will serve to illustrate the power of the large \upgradetwo data set to distinguish between different NP models. This power relies critically on the ability to exploit multiple related decay channels. 
...
By comparison to the systematics limited branching fractions, a better comparison between theory and experiment can be achieved by studying isospin and \CP asymmetries, which with the \upgradetwo data set will be experimentally probed with percent level precision. The \upgradetwo data set will also enable new decay modes to be studied, for example higher spin-\Kstar states and modes with larger numbers of decay products. It is also possible to reduce theoretical and experimental uncertainties by comparing regions of angular phase-space of $\bquark\to\squark \ellell$ decays. The angular distribution of \decay{\B}{V \ellell} decays, where $V$ is a vector meson, can be expressed in terms of eight \qsq-dependent angular coefficients that depend on the Wilson coefficients and the form-factors. 
...
Thanks to LHCb's particle identification capabilities, the \upgradetwo data set will provide a unique opportunity to make precise measurements of $\bquark\to\dquark \ellell$ processes.
