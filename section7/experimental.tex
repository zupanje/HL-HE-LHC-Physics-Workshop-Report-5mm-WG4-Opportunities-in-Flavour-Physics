...
bins of \qsq or using a \qsq-unbinned approach~\cite{Hurth:2017sqw}; LHCb foresees to achieve around 440\,000 fully reconstructed \decay{\Bz}{\Kstarz\mumu} decays, CMS around 700\,000 excluding the $q^2$ range 
 overlapping with the resonant decays. For LHCb the measurement is currently heavily statistically limited, and the major systematics are those which affect the calculation of the angular acceptance. Most of the systematic uncertainties are expected to reduce significantly with more integrated luminosity due to larger control samples, and are therefore scaled with the integrated luminosity. In this context it is interesting to note that \upgradetwo will provide signal yields that are of the order of the current tree-level control modes $B^0\to \jpsi \Kstarz$ and $B^0_s\to \jpsi\phi$. The most recent determination of $\phis$ via a time-dependent angular analysis of $B^0_s\to \jpsi\phi$ the current systematic uncertainties on the decay amplitudes due to modeling of the angular acceptance are already at the 0.001–0.002 level which would be sufficient to not systematically limit the \upgradetwo analysis.
...
\par\noindent    
The increased amount of collected data foreseen for the Phase 2 integrated luminosity offers the opportunity to perform the angular analysis in
 ...
 The dimuon mass resolution is measured on the MC simulation as a function of $q^2$. Each systematic uncertainty in the Run 1 analysis bin is 
