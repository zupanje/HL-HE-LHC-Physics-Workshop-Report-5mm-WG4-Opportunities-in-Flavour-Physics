...
The remaining analysis-dependent systematic uncertainties are combined together by ATLAS and parameterized as a function of the signal yield, while e.g. CMS provides the detailed breakdown shown in table~\ref{tab:CMSb2musys}. The resulting HL-LHC projected statistical and systematic uncertainties for the three experiments are summarized in table~\ref{LHC_Bmumu_Reach}.

\begin{table}
\centering
\caption{CMS systematic uncertainties for Phase II  scenario.}
\label{tab:CMSb2musys}
\begin{tabular}{cc}\hline
Source & Uncertainty \\ \hline 
Muon ID efficiency ratio & 1\% \\ 
$\mathrm{B^+}$ normalization yield & 1.4\% \\
Peaking background yield & 10\% \\ 
Semileptonic background yield & 7.5\% \\
fs/fu ratio & 3.5\% \\$\mathrm{B^0_s}$ Effective lifetime & 2\% \\
...
\begin{table}
  \centering
  \caption{Projected ATLAS uncertainty on $\cal{B}$$(B^0_s \to \mu^+ \mu^-)$ and $\cal{B}$$(B^0 \to \mu^+ \mu^-)$. The results are centered on the SM theoretical prediction~\cite{Bobeth}.
...
to the HL-LHC expectations. The effective lifetime statistical uncertainty is projected to
be 0.15 ps for the Run 2 dataset and 0.05 ps at HL-LHC.

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.5\textwidth]{\main/section7/figures/CMS_tau_splot_toy_bs.pdf}
\caption{The CMS binned maximum likelihood fit to the decay time distribution for the Phase-II scenario (taken from \cite{CMS-PAS-FTR-18-013})}
...
the sum of two Gaussian distributions and the background with an exponential distribution. 
  The projected statistical uncertainty on the $\mathrm{P'_5}$  parameter measurement is obtained scaling the statistical uncertainty measured in Run 1 
...
Uncertainties concerning the simulation mismodeling, the efficiency determination,
 ...
 of the number of events.The uncertainty due to the finite size of the simulation is considered to be zero in the HL-LHC  scenario.
 The resulting $P_5'$ uncertainties in the $q^2$ bins are estimated to improve up to a factor of 15 in the HL-LHC era with respect to those quote in the Run~1 analysis.
...
\caption{Experimental sensitivity to the $P_5^{\prime}$ angular observable in the SM,
  ...
  The sensitivity is computed assuming that the charm-loop contribution is determined from the data. {\bf\color{red}[What does this mean? What do these errorrs contain? Experimental only?]}
}
