2\subsection{Experimental prospects}
...
Unfortunately at \lhcb, the reconstruction efficiency for the dominant $\Pchi_{\cquark 1}(1P)$ decay  to \g\jpsi decay is low, making this prediction hard to test.
 The very large data set of LHCb \upgradetwo  will allow to overcome this problem. 
The very large data set of the LHCb \upgradetwo will be essential
...
The determination of their properties, or even claim of their existence, relies on advanced amplitude analysis,
...
In the case of 
%the existence of 
an isospin quadruplet, there is the interesting possibility to find doubly charged
...
Since the \Dz and \Dsp mesons are pseudoscalars, the amplitude analysis simplifies, 
 and can provide conclusive results already with few hundred decays.
...
driven by tightly bound tetraquark dynamics. \fk{FK: the prompt production could be driven by the $b\bar b$ Fock state component of the same quantum numbers. The production cross sections may reach the level of nb~\cite{Guo:2014sca}.}
...
Such open-beauty excited states could be searched for in decays to \Xib\kaon final state.
...
\subsubsection{Study of doubly-heavy baryons}
  ...
  Improvements in the trigger for the upgraded \lhcb detector \td{JZ: which upgrade} are projected to
  ...
  more~\cite{LHCb-TDR-012,LHCb-PAPER-2012-031}.
	Thus the  Run 5 \td{are we using \upgradetwo?} sample  will contain more than 90\,000 decays of this mode.
  ...
  \Omegacc~\cite{Kiselev:2001fw}, \lhcb will have Run 5 \td{ \upgradetwo?} yields of around 25\,000 for \Xiccp and 4500 for \Omegacc in each of several decay modes.
  ...
  exhausted by the end of Run 5 \td{ \upgradetwo?}.
  ...
  Run 5  \td{ \upgradetwo?}will supply precision measurements of doubly differential
  cross-sections that will provide insight into production
  mechanisms of doubly heavy baryons. In addition 
  Run 5  \td{ \upgradetwo?} will allow the spectroscopy of excited states and bring studies of the
...
in expected in Run 5 \td{\upgradetwo?}, albeit with sizeable uncertainties.
Other modes could also provide sizeable signal samples.
It is likely \lhcb will observe the $\Xires_{\bquark\cquark}$ baryons in Run 3/4 \td{defined?}, and further probe the spectrum of other doubly heavy baryons with the large samples accessible in the proposed \upgradetwo. 
...
Use of Run 5 \td{upgradetwo?} dataset is also necessary  to measure the $(\pt,y)$ dependence of $\Pchi_\cquark$ polarisation parameters. 
...
Though the larger samples collected during  Run~3 and 4 \td{defined?} will allow some progress on these questions, the measurement of the correlation of \jpsi polarisation parameters will only be possible with the \upgradetwo data set.
...
the determination of the relative SPS and DPS contributions, as well as the discrimination between different SPS theory models, will require precision measurements only possible with  Run 5 \td{\upgradetwo?} data.
