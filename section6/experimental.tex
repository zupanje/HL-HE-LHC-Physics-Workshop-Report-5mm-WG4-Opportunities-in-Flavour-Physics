...
% This anecdotally speaks against compact interpretations for the other states.
...
%distinguish between several of the models that attempt to explain the structure of the observed pentaquarks~\cite{Wang:2016dzu}. Experimentally, the measurement of the magnetic moments is challenging.
%Access can be gained
%through radiative decays of pentaquark states. %\fk{FK: I am confused about this point. I think the radiative decays measure the transition form factors instead of the magnetic moments. Magnetic moments requires to measure the form factor  for $\langle P_c|J_\mu|P_c\rangle\rangle$, where $J^\mu$ is the em current, with the same initial and final states. Is this feasible?}
%The observation of radiative decays
...
%\jpsi\proton{}\g\kaon decay. The proposed improved ECAL is crucial for the
%feasibility of such a measurement in a high-luminosity environment.
%
%To get an idea about the typical suppression
%of radiative decays with respect to hadronic decays we can look at the measured branching fractions for
...
%Branching fractions of ${\mathcal O}(1\%)$ (\eg $\BF(\decay{\Lz(1520)}{\Lz \g})=0.85 \pm 0.15\% $)
...
%or a quadruplet with $I=\frac{3}{2}$. \fk{FK: The decays of $P_c$ into the $J/\psi p$ happen through strong interaction. Thus, the isospin itself is also a good quantum number to a very good approximation. If the isospin of $P_c$ is 3/2, then the decay into $J/\psi p$ breaks isospin symmetry, and should be highly suppressed. To have a feeling, it should be suppressed in compasion with a normal isospin-conversing strong decay width roughly by a factor of $((m_d-m_u)/\Lambda_\text{QCD})^2\sim 1/\alpha^2\sim 10^{-4}$. In my opinion, $I=3/2$ is unlikely.}
%In both cases 
...
%\fk{FK: the prompt production could be driven by the $b\bar b$ Fock state component of the same quantum numbers. The production cross sections may reach the level of nb~\cite{Guo:2014sca}.}
