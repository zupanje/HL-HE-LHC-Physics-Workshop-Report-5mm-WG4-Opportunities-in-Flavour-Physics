...
\section[Strange-quark probes of new physics]{Strange-quark probes of new physics\footnote{Author(s) (TH): Giancarlo D'Ambrosio, Diego Guadagnoli, Teppei Kitahara,  Diego Martinez Santos.}}
...
The large uncertainty comes from the LD contribution which  has been computed in ChPT \cite{Ecker:1991ru}. This uncertainty 
 is expected to be reduced by a dispersive treatment of $K_S \to \gamma^{\ast} \gamma^{\ast}$ \cite{Colangelo:2016ruc}, where  $K_S \to \gamma \gamma$, $K_S \to  \mu^+ \mu^- \gamma $, $K_S \to \mu^+ \mu^- e^+ e^-$  and $K_S \to \mu^+ \mu^- \mu^+ \mu^-$ are measurable in LHCb experiment and KLOE-2 experiment at DA$\Phi$NE \cite{AmelinoCamelia:2010me}. \Blu{It is important to note that, in contrast to the $K_L\to\mu^+\mu^-$ decay:\begin{itemize}
\item[\bullet]
...
\item[\bullet]
As seen in (\ref{eq:KSmumu:SM}) the long-distance (LD) and short-distance (SD) contributions to the total rate are added incoherently \cite{Ecker:1991ru, Isidori:2003ts}, which represents a big theoretical advantage over $K_L\to\mu^+\mu^-$, where LD and SD amplitudes interfere.
\end{itemize}

This means that in BSMs in which the the SD contribution is significantly enhanced, the LD-uncertainty in $K_S\to\mu^+\mu^-$ ceases to be important, and theoretically clean tests of new-physics scenarios are possible. In particular, being the SD contribution dominant, correlations of $K_S\to\mu^+\mu^-$ with $\varepsilon' /\varepsilon$ and also $K_L\to\pi^0\nu\bar\nu$ are present within many new-physics models.
...
%It is found that t
the interference of $CP$-violating contribution can affect $K_S \to \mu^+ \mu^-$ decay up to $\mathcal{O}(60)\%$. \Blu{This is of current interest given the possible discrepancies with the SM in $\varepsilon^\prime/\varepsilon$~\cite{Pallante:1999qf,Buras:2015yba,Buras:2015xba,Kitahara:2016nld,Gisbert:2017vvj}.}
...
{\main/section4/img/BrKSmumu_noGreen.pdf}
...
SM prediction with (without) taking into account the dilution $D$. 
...
Such test are especially interesting in view of the $B$-physics anomalies (see Sec.~\ref{secBanom}), with rare kaon decays providing a complementary role in testing the NP explanations.  Our analysis~\cite{Crivellin:2016vjc} is based on the observation that at low energy scales, $\mu \ll m_{t,b,c}$, 
