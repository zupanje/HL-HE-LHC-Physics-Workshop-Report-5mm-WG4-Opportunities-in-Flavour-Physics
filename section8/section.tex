...
The SM predictions are thus controlled by the flavor couplings of the top -- with $B$ and $K$ transitions determining the CKM matrix elements $V_{tb}, V_{ts}, V_{td}$ through this virtual effects. Determining these and the structure of the $Wtb$ vertices  directly, from high $p_T$ transitions, can then serve as independent consistency checks of the SM. 

While the LHC is acknowleged to be a top factory and the currently available statistics has already allowed ATLAS and CMS to perform a vast campaign of top related measurements the larger number of produced top quarks at HL-LHC will open new perspectives for precise measurements of top-quark properties and for NP probe by improving the sensitivity to rare decays such as those involving flavor changing neutral currents.

\subsection{Global effective-field-theory interpretation of top-quark FCNCs} \label{sec:top:FCNC} {\bf Author(s):} Cen Zhang, Gauthier Durieux
{
...
Starting from and Effective Field Theory (EFT) with full $SU(3)_C\times SU(2)_L\times U(1)_Y$ gauge symmetry and matter content of the SM,
...
The corresponding Warsaw-basis operators have been listed in Eqs.\,(12-20) and (21-28) of Ref.~\cite{AguilarSaavedra:2018nen}.
They are:
\begin{equation}
	\begin{aligned}
	\hc{\qq{}{u\varphi}{ij}}
...
\begin{aligned}
  ...
  \end{aligned}
\end{equation}
Note however that the $\qq{}{\varphi ud}{}$, $\qq{}{dW}{}$ and $\qq{}{ledq}{}$ operators only contribute to charged top-quark currents {\color{red} (when one does not consider SM electroweak corrections)} and are therefore not relevant for our purpose.
...
experimental analyses (Ref.~\cite{Achard:2002vv} excepted).  
...
are actually related to those relevant to top-quark physics (see Secs.~\ref{}).
...
\item[$\bullet$] For top-quark decay to a Higgs boson and a jet, we use the most stringent limits set by ATLAS in the multilepton channel. The dependence on all operator coefficients but $C_{t\phi}$ and $C_{tG}$ is assumed negligible.

\item[$\bullet$] Limits on $e^+e^-\to tj+\bar{t}j$ obtained at LEP~II~\cite{Aleph:2001dzz} still set the dominant constraints on four-fermion operators involving electrons while $t\to q\ell\ell$ at hadron colliders are the only measurements constraining four-fermion operators featuring a pair of muons.
...
\begin{itemize}
\item[$\bullet$] As previously, we assume that the limits quoted on the $\br(t\to qZ)$ branching fraction are derived using the dilepton decays of the $Z$ boson only and in a $m_{\ell\ell}\in[78,102]\,\gev$ window for the dilepton invariant mass. This determines the sensitivity to four-fermion operators.

\item[$\bullet$] The limits on the $tq\gamma$ interaction have been derived by a combination of production and decay processes~\cite{CMS:2017gvo}. As the only prospect provided is on the $\br(t\to q\gamma)$ branching fraction, we approximate it as arising that arise from the measurement of the decay process only. The difference would lie primarily in the dependence on $tqg$ interactions.
...
{\bf Author(s):}Frederic Deliot, Wouter Dekens
...
\end{align}
It should be mentioned that, within the SMEFT, gauge invariance links the above vertices to additional interactions, the full forms of the relevant operators are given in Sec.~\ref{sec:top:FCNC}.
...
Here we briefly discuss both the direct and indirect limits on the operators in Eq.\ \eqref{eq:Wtb}, as well as the projections for the HL-LHC.
...
%
It should be mentioned that the single-top cross sections can receive contributions from other operators in the SMEFT, in particular from several four-quark operators. Thus, in order to perform an exhaustive global analysis one would have to include these effects as well. For discussions on such operators in the context of single-top production, see e.g.\ Refs.\ \cite{Buckley:2015lku,Buckley:2015nca}.
...
It should be noted that top decays can in principle be used to probe physics beyond that of the top sector. For example, it was recently suggested \cite{Harrison:2018bqi} that the process $t\to bW\to b\bar b c$ can used to measure $V_{cb}$ at the $m_W$ scale, instead of $V_{cb}(m_b)$ which is probed in $B$ decays.
\newline
...
These limits were obtained by assuming that $c_{\varphi q}^-=0$.~\footnote{This coupling is harder to constrain as it is degenerate with a shift in $V_{tb}$, see Eq.\ \eqref{eq:Wtb}. This degeneracy can be broken by using CKM unitarity tests, or by considering the additional interactions, such as the $Wts$, $Wtd$, and $Ztt$ vertices, that are linked to $c_{\varphi q}^-$ by gauge invariance. A more naive constraint, obtained by setting $V_{tb}=1$, leads to $|c_{\varphi q}^-|\lesssim 1$ \cite{Alioli:2017ce}.}
%
...
\begin{table}
\center\small
\caption{Indicative constraints on the SMEFT operators contributing to the $Wtb$ vertex from indirect observables. A single Wilson coefficient was turned on at a time and we assumed $\Lambda = 1$ TeV.}
\vspace{.2cm}
...
We will nevertheless consider the limits in the case that a single dimension-six operator is generated at the BSM scale.
...
Of the operators in Eq.\ \eqref{eq:Wtb}, the $c_{\varphi tb}^{[I]}$ and $c_{bW}^{[I]}$ couplings are mainly constrained by the neutron EDM, while $c_{ tW}^{[I]}$ contributes to the electron EDM. $c_{\varphi tb}^{[I]}$ and $c_{bW}^{[I]}$ first generate the bottom-quark chromo EDM,  $O_{dG}^{(33)}$ (in the notation of \cite{Grzadkowski:2008mf}), through one-loop diagrams \cite{Dekens:2013zca,Alonso:2013hga,Alioli:2017ce}, which subsequently induces the Weinberg operator, $O_{\tilde G}$, after integrating out the bottom quark \cite{Dicus:1989va,Weinberg:1989dx,BraatenPRL,Boyd:1990bx}. The hadronic matrix element of the contribution of the Weinberg operator to the neutron EDM is poorly known. Combining naive-dimensional-analysis and sum-rule estimates \cite{Pospelov_Weinberg,Weinberg:1989dx,deVries:2010ah} one has $ |d_n|  = 6|e\,(50\, {\rm MeV})\, C_{\tilde G}(1\, {\rm GeV})|$, with an $\mathcal O(100\%)$ uncertainty.
...
{\bf Authors:} Mariel Estevez, Darius Faroughy, Jernej Kamenik 
%
% 
%Fabio Maltoni
...
missing energy ($E^{miss}_{T}$). The analysis at CMS requires exactly one b and one non-b jet to b present in the final state with the dominant background arising from the $t\bar{t}$+jets
