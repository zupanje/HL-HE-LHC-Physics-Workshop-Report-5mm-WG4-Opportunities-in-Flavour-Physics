...
Top quark is distinguished by its large mass and its ${\mathcal O}(1)$ coupling to the Higgs, quite distinct from any other SM fermion. It stands to reason that exploring top quark  properties may shed light on the resolution of the SM flavour puzzle, or at least as to why one and only one Yukawa coupling is large. At the same time, the large coupling of top to the Higgs is also the reason for the weak scale hierarchy problem to be so acute -- the quadratic divergent corrections to the Higgs mass are driven almost completely by this coupling. BSM models addressing the hierarchy problem may thus well leave an imprint in the top quark properties and decays.  For instance, the top FCNCs, $t\to c\gamma, cZ, cg$ are null tests of the SM and are used as BSM probes.
%plays a central, often secluded, role in Flavor Physics. It is the only quark with unsuppressed couplings to the Higgs boson and, therefore, it is furnished with a large mass, of the order of the EWSB scale. 

% makes the top-quark crucial in addressing the Flavor Puzzle (Sec.~\ref{sec:Intro}) and potentially sensitive to BSM addressing it. 
% Large Yukawa couplings lead to large top-loop corrections and the accuracy of theoretical predictions rely on a precise measurements of the top-quark mass. This is important for phenomenology, as in Higgs production and diphoton searches, and has far and profound theoretical consequences related to the stability of the mass of the Higgs, or the stability of the Universe itself (the famous $\Lambda_{\textgoth{Hell}}$). 
...
%and, as such, it is essential for the SM predictions and provides a natural conduit for BSM affecting rare $B$-meson and kaon decays. 
The SM predictions are thus controlled by the flavor couplings of the top -- with $B$ and $K$ transitions determining the CKM matrix elements $V_{tb}, V_{ts}, V_{td}$ through this virtual effects. Determining them directly, from high $p_T$ transitions, can then serve as independent consistency checks of the SM.
%or to the structure, and possible $CP$ violation, of the $Wtb$ couplings. They are also directly sensitive to BSM affecting top FCNCs, which are null tests of the SM. 
...
Of the operators in Eq.\ \eqref{eq:Wtb}, the $c_{\varphi tb}^{[I]}$ and $c_{bW}^{[I]}$ couplings are mainly constrained by the neutron EDM, while $c_{ tW}^{[I]}$ contributes to the electron EDM. The contributions to the neutron EDM arise through the Weinberg three-gluon operator, $O_{\tilde G}$, and the bottom-quark chromo EDM, $O_{dG}^{(33)}$, in the notation of \cite{Grzadkowski:2008mf}. In the first step $c_{\varphi tb}^{[I]}$ and $c_{bW}^{[I]}$  generate the bottom-quark chromo EDM through one-loop diagrams \cite{Dekens:2013zca,Alonso:2013hga,Alioli:2017ces}, which induces the Weinberg operators ater integrating out the bottom quark \cite{Dicus:1989va,Weinberg:1989dx,BraatenPRL,Boyd:1990bx}. The hadronic matrix element of the contribution of the Weinberg operator to the neutron EDM is poorly known. Combining naive-dimensional-analysis and sum-rule estimates \cite{Pospelov_Weinberg,Weinberg:1989dx,deVries:2010ah} one has $ |d_n|  = 6|e\,(50\, {\rm MeV})\, C_{\tilde G}(1\, {\rm GeV})|$, with an $\mathcal O(100\%)$ uncertainty. 
