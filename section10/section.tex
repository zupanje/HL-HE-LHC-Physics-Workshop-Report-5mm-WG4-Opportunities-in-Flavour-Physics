...
%,
%
and again lepton number is approximately conservation at low energies.
...
\\[2mm]
\includegraphics[width=0.45\textwidth]{\main/section10/img/lhcSeesaw_Vta2_Vs_mN_taelX_LHCX14_1805_09335.pdf}~~~~~
%	\label{fig:jetvetotael}
\includegraphics[width=0.45\textwidth]{\main/section10/img/lhcSeesaw_Vta2_Vs_mN_tataX_LHCX14_1805_09335.pdf}%	\label{fig:jetvetotata}
%\\
...
%from \cite{Ruiz:2017yyf}.
Lower:
Sensitivity to the active neutrino--heavy neutrino mixing, $\vert V_{\ell N}\vert^2$, as a function of the heavy neutrino mass, $m_N$, in the 
tri-lepton final states 
 $\tau_h^\pm e^\mp \ell_X + \textrm{MET}$ (left), and 
 $\tau_h^+ \tau_h^- \ell_X + \textrm{MET}$ (right), 
...
Projections from a new search strategy recently proposed in \cite{Pascoli:2018rsg}, based on a dynamical jet veto selection cut,
%found an increased sensitivity to active-heavy neutrino mixing by approximately an order of magnitude over the LHC's life.
%This is 
are shown in Fig. \ref{fig:lhcSeesaw_ISS} (lower) for
 $\tau_h^\pm e^\mp \ell_X + \textrm{MET}$ and
 $\tau_h^+ \tau_h^- \ell_X + \textrm{MET}$ final states,
with $\tau_h$ the hadronically decaying $\tau$ and $\ell_X\in\{e,\mu,\tau_h\}$.
With $3\,\mathrm{ab}^{-1}$  the HL-LHC will be able to probe heavy neutrino masses up to $350\,\mathrm{GeV}$, 
%and mixings down to $\vert V_{\ell N}\vert^2\simeq 10^{-3}$, 
taking into account global constraints on the active-heavy mixing~\cite{Fernandez-Martinez:2016lgt} (dot-dashed line in Fig. \ref{fig:lhcSeesaw_ISS} (lower)),
% could be probed.
