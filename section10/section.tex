...
In most solutions to the SM flavour puzzle, Yukawa couplings have a dynamical origin. 
Their cosmological evolution should be studied.
 In Ref.~\cite{Baldes:2016rqn,Baldes:2016gaf,vonHarling:2016vhf,Bruggisser:2017lhc,Bruggisser:2018mus,Bruggisser:2018mrt,Baldes:2018nel,Servant:2018xcs},  the connection between flavour cosmology and electroweak baryogenesis has been investigated.

Electroweak baryogenesis (EWBG) is a mechanism to explain the matter antimatter asymmetry of the universe  using Standard Model baryon number violation. It 
 relies on a charge transport mechanism in the vicinity of bubble walls during a first-order electroweak (EW) phase transition. It uses EW scale physics only and is therefore testable experimentally. It requires an extension of the Higgs sector leading to a first-order EW phase transition as well as new sources of CP violation. A typical constraint on EWBG comes from Electric Dipole Moments.
 ...
 This is  precisely what can happen in Composite Higgs models ~\cite{Kaplan:1983fs,Panico:2015jxa}, on which we focus in the following.


%Intro: CH

These models assume that the Higgs boson arises as a bound state of a new strong interaction, confining around the TeV scale $f$. Other composite resonances are naturally heavier than the Higgs, as a result of the approximate Goldstone symmetry suppressing the Higgs mass. The rest of the SM fields do not belong to the strong sector and are elementary. The nature of the phase transition in CH models can be substantially different with respect to the SM. One of the reasons is that these models can naturally feature new scalar resonances, participating in the EW phase transition and changing its properties (see Refs.~\cite{Espinosa:2011eu,Chala:2016ykx}). On the other hand, the EW transition may become strong without help of additional states, if is happens simultaneously with the deconfinement-confinement phase transition of the new strong sector (see Refs.~\cite{Bruggisser:2018mus,Bruggisser:2018mrt} and also Refs.~\cite{vonHarling:2016vhf,Megias:2018sxv} for the dual description in the warped 5D space).    

%Flavour in CH

The flavour structure of the CH models has a paramount importance for the viability of EWBG. Requirement to produce the sizeable top quark Yukawa coupling, while suppressing the unwanted flavour-violating effects suggests that the new sector is nearly scale-invariant
% and strongly coupled 
for a large range of energies above the confinement scale. As a result, one may expect that the transition dynamics is mostly determined by a single light field -- the dilaton $\chi$ (see e.g Ref.~\cite{Coradeschi:2013gda}). Depending on its mass (whose size can be related to the separation of the UV flavour scale and the EW scale), the EW phase transition may happen separately from the confinement, or together~\cite{Bruggisser:2018mus,Bruggisser:2018mrt}. %The two-step transition is expected to happen for the heavier dilaton, while the one-step -- for the dilaton lighter than about $\sim 0.5$~TeV. 

Moreover, the mechanism of generation of the SM flavour hierarchy in CH models may eventually also be a source of CP asymmetry during the EW phase transition. The hierarchy of SM Yukawas $\lambda_q$ is generated by the renormalization group running of the couplings $y_q$ between the elementary fermions and the strong sector operators: 
\begin{equation}
...
\caption{Relative deviation of the Higgs couplings to $W$ and $Z$ bosons (left panel) and the imaginary correction to the top quark Yukawa coupling (right panel), in terms of the dilaton mass $m_\chi$, the number of colours of the new confining sector $N$, and the mass of generic composite states $m_*$. Black contours correspond to the glueball-like dilaton and red dashed to the meson-like dilaton. The current  and near future experimental sensitivities of electron EDM experiments  to the imaginary part of the top yukawa correspond respectively to approximately $2\times 10^{-2}$~\cite{Cirigliano:2016nyn} and $2\times 10^{-4}$~\cite{Kumar:2013qya}.
}
\label{fig:ewbg_observables}
\end{figure}

For both types of transitions  mentioned, one can expect to observe deviations of the Higgs couplings from the Standard model predictions. These deviations, besides containing contributions which are generic for CH models, also contain those which are linked to the features of the phase transition and the new source of CP-violation. For concreteness, let us focus on the case with a combined electroweak and strong sector phase transition as a more minimal one. 
%
...
To generate a sufficient amount of CP violation, such a mass mixing has to be sizeable. Let us consider its effect on the quark Yukawa coupling~\footnote{Similar, but CP-preserving, modifications are also generated in the couplings of the Higgs boson to the SM gauge fields and the Higgs self-interactions.}:
\begin{equation}\label{eq:ewbg_yuk}
{\cal L}_{Yukawa} = \lambda(\chi) \left(\chi \sin \frac h f \right) \bar q_L q_R 
= \bar q_L q_R h \left(\lambda(f) \frac {\chi}{f} + \beta_{\lambda} \frac {\chi - f }{f} \right) + \dots 
\end{equation}
where we performed an expansion in $\chi$ around its today value $f$.
The complex phase of the Yukawa beta-function $\beta_\lambda$ has to be different from that of the quark mass $\lambda(f) h$, as the Yukawa phase changes with energy. 
Choosing the mass parameter to be real, the CP-violating interaction becomes localised in the term $\propto \beta_\lambda$. Now, rotating the $h$ and $\chi$ fields to the mass basis, one finds a CP-even and odd corrections to the Higgs Yukawa interaction
\begin{equation}
