...
The composite Higgs models are described by the compositness scale $f$ and the mass of the first composite resonances, $M_*\sim g_* f$, with $g_*$ the typical strength of the resonances in the composite sector. The searches at the HL-/HE-LHC consist of Higgs coupling measurements, including deviations in Higgs Yukawa couplings \td{cite WG2}, and searches for composite resonances \td{cite WG3}.
...
The SM has in the limit of vanishing Yukawa couplings a large global symmetry. In the quark sector this is $G_F=SU(3)_Q\times SU(3)_U\times SU(3)_D$. Ref. \cite{Grinstein:2010ve} showed that the SM flavor symmetry group $G_F$ is anomaly free, if one add a set of fermions that are vector-like under the SM gauged group, but chiral under the $G_F$. This means that $G_F$ can be gauged. It is broken by a set of scalar fields that have hiearchical vevs and lead to hierarchy of SM quark masses. This also implies a hierarchy  for the masses of the flavoured gauge bosons as well, but with gauge bosons that more strongly couple to third generation being lighter, while the flavoured gauge bosons that couple more strongly to the first two generations are significantly heavier. This pattern in the spectrum of flavoured gauge bosons then avoids too large FCNCs.  
%\td{to be done}

At the LHC one searches for the lightest flavoured gauge bosons, of ${\mathcal O}({\rm TeV})$ mass, which couple mostly to $b$ quarks and $t$ quarks, but could also have non-negligible couplings to the first two generations. The di-jet and $t\bar t$ resonance searches are thus sensitive probes. A signal could also come from production of the lightest vectorlike fermions, $t'$, or $b'$. For several benchmarks see \cite{Grinstein:2010ve}, and also, e.g., Ref. \cite{Bishara:2015mha} (where also a connection with dark matter was explored).

\subsubsection{2HDM and low scale flavour models}
\label{sec:2HDM:flavour}
%taken from WG2, should cite them
{\bf Authors: Martin Bauer, Marcela Carena and Adri\'an Carmona}
In 2 Higgs Doublet Models (2HDMs), the two Higgs doublets, $H_1$ and $H_2$ are usually considered not to carry flavour quantum numbers. The collider phenomenology, on the other hand, changes substantially, if they do. 
...
+\mathrm{h.c.}\,,
\end{align}
where $\tilde{H}_1\equiv i\sigma_2 H^{\ast}_1$ as usual, and the charges $n_{u,d,e}$ are a combination of the $U(1)$ charges of $H_1$, $(H_1H_2)$ and the different SM fermion fields. For simplicity, we set the flavour charges of $H_1$ and $H_2$ to $1$ and $0$, respectively, such that  
%\begin{align}
$n_{u_{ij}}=a_{q_i}-a_{u_j}, n_{d_{ij}}=a_{q_i}-a_{d_j}, n_{e_{ij}}=a_{\ell_i}-a_{e_j}$,
%\end{align}
...
\end{align}
with the vacuum expectation values $\langle H_{1, 2}\rangle=v_{1, 2}$ and $t_\beta \equiv v_1/v_2$. Besides being able to accommodate the observed hierarchy of SM fermion masses and mixing angles for the right assignment of flavour charges \cite{}, this framework can lead to enhanced diagonal Yukawa couplings between the Higgs and the SM fermions while having suppressed FCNCs. If we denote by $h$ and $H$ the two neutral scalar mass eigenstates, with $h$ the observed $125$ GeV Higgs, the couplings between the scalars $\varphi=h,H$ and SM fermions $\psi_{L_i, R_i}= P_{L,R} \psi_i$ in the mass eigenbasis read 
...
\end{align}
The flavour universal functions in \eqref{eq:diagocoup} and \eqref{eq:foffc} read
\begin{align}
...
$D^0-\bar{D}^0$ oscillations constrain the products of two $S$ Yukawas, $\tilde Y_{ut} \tilde Y_{ct(tc)}$, and $\tilde Y_{tu} \tilde Y_{ct(tc)}$, to be small~\cite{Bona:2007vi,Harnik:2012pb,Agashe:2013hma}. This means that $S\bar t c$ or $S\bar t u$ couplings can be sizeable, but not both at the same time. Therefore, inspired by the CMS $t\to hc$ search~\cite{CMS:2017cck}, Ref.~\cite{Banerjee:2018fsx} developed a dedicated analysis for $t\to qS$, by varying $m_S$. The projections for  $S \to b\bar{b}$ and $S \to \gamma \gamma$ decays, at the 14 TeV LHC, are shown in Figs. \ref{fig:sbb2} and \ref{fig:sgg2}, respectively.  Note that very light $S$, i.e., with $m_S < m_h/2\sim 62.5$ GeV, need not be
...
\end{equation}
where $f$ is the NP scale, and on r.h.s. we introduced a new flavour violating coupling $\tilde g$. The three Benchmark Points (BP) shown in Figs. \ref{fig:sbb2} and \ref{fig:sgg2} are
...
The left panel in Fig.~\ref{fig:sbb2} shows  the $3$ ab${}^{-1}$ \td{correct?}
...
\begin{figure}[t]
 ...
 %
 \caption{\label{fig:sbb2} \textit{Left:} Branching ratios that can be tested in the $b\overline{b}$ channel. Superimposed are the theoretical expectations in the three BPs. \textit{Right} Luminosity required to test $\mathcal{B}(t\rightarrow S c, S\rightarrow b\overline{b}) = 10^{-4}$. Superimposed are $\mathcal{L} = 300$ fb$^{-1}$ and $3000$ fb$^{-1}$.}
\end{figure}
%
%
%
...
In regard of the possibilities at future colliders, we find that the increase in cross-section 
...
%taken from WG2, should cite them
{\bf Authors: Martin Bauer, Marcela Carena and Adri\'an Carmona} Interestingly, in some models the flavour structure can feed back into nontrivial constraints on the scalar potential. This was demonstrated in the 2HDM model with FN charges, introduced in Section \ref{sec:2HDM:flavour}.
...
%
 \begin{figure}
 \begin{center}
% \includegraphics[width=.5\textwidth]{\main/section10/img/BRvskap.pdf}
...
{\bf Authors T. Han, T. Li, X. Marcano, S. Pascoli, R. Ruiz, C. Weiland}
%need to cite WG3
%{\bf Author(s): 
...
The potential Majorana nature of neutrinos as well as the origin of their tiny masses and large mixing angles remain some of the most pressing open issues in particle physics today.
Models that address these mysteries, collective known as Seesaw models, hypothesize the presence of new particles that couple to SM fields via
...
and greatly complement low energy and oscillation probes of neutrinos~\cite{Atre:2009rg,Cai:2017mow}.
In the following, we summarize the discovery potential of Seesaw models at hadron colliders with collision energies of $\sqrt{s} = 14$ and 27.
...
\paragraph*{The Type I Seesaw and Variants Discovery Potential at the HL- and HE-LHC}\label{sec:lhcSeesawtype1}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The Type I Seesaw hypothesizes the existence of fermionic gauge singlets with a Majorana mass term whose couplings 
to SM leptons generate light neutrino masses and mixing. 
...
In other words, for low-scale seesaw models with only fermionic singlets, 
lepton number has to be nearly conserved and light neutrino masses are proportional to small lepton number violation (LNV) parameters in these models. 
...
and again leads to approximate lepton number conservation at low energies.
This in turn leads to the expectation that LNV processes should be suppressed in variants of the type I seesaw unless additional particles,
whether they be fermions or scalars charged under the SM gauge couplings or 
new gauge interactions,
are introduced to decouple the light neutrino mass generation from the heavy neutrino production.
...
If kinematically accessible, heavy neutrinos $N$ can generically be produced in hadron collisions through both neutral current and charged current processes,
as shown in Fig. \ref{fig:lhcSeesaw_HeavyNProd}(upper).
...
\begin{figure}[!t]
\begin{center}
%\includegraphics[width=0.45\textwidth]{\main/section6FlavorRelatedStudies/img/lhcSeesaw_Vta2_Vs_mN_taelX_LHCX14_1805_09335.pdf}	\label{fig:jetvetotael}
%\includegraphics[width=0.45\textwidth]{\main/section6FlavorRelatedStudies/img/lhcSeesaw_Vta2_Vs_mN_tataX_LHCX14_1805_09335.pdf}	\label{fig:jetvetotata}
%\\
...
\caption{
Upper:
Sensitivity to the active-heavy mixing $\vert V_{\ell N}\vert^2$ as a function of the heavy neutrino mass $m_N$ in the 
tri-lepton final states 
(L) $\tau_h^\pm e^\mp \ell_X + \textrm{MET}$ and 
(R) $\tau_h^+ \tau_h^- \ell_X + \textrm{MET}$, 
assuming $\vert V_{e4}\vert^2 = \vert V_{\tau4}\vert^2$ and $\vert V_{\mu4}\vert^2=0$, at the $\sqrt{s}=14\,\mathrm{TeV}$ LHC.
The dash-diamond line corresponds to the standard analysis with a b-jet veto  while the solid-star line is the jet veto-based analysis~\cite{Pascoli:2018rsg}.
Lower:
...
The expected suppression of LNV processes in models that contain only fermionic gauge singlets
motivates the study of lepton number conserving (LNC) processes. 
A first possibility to consider is the trilepton final state $\ell^\pm_i \ell^\mp_j \ell^\pm_k + \textrm{MET}$.
The heavy neutrino $N$ here is produced via both charged-current DY and VBF, and subsequently decays to only leptons, i.e.
\begin{equation}
 p p \to \ell_N N +X \to \ell_N \ell_W W +X \to \ell_N \ell_W \ell_\nu \nu +X\,.
 \label{eq:pp3lX}
\end{equation}
Notably, a new search strategy was recently proposed in \cite{Pascoli:2018rsg} based on a dynamical jet veto selection cut
and found an increased sensitivity to active-heavy neutrino mixing by approximately an order of magnitude over the LHC's life.
This is shown in Fig. \ref{fig:lhcSeesaw_ISS}(upper) specifically for the final states 
(L) $\tau_h^\pm e^\mp \ell_X + \textrm{MET}$ and
(R) $\tau_h^+ \tau_h^- \ell_X + \textrm{MET}$,
where $\tau_h$ represents a hadronically decaying $\tau$ and $\ell_X\in\{e,\mu,\tau_h\}$.
With $3\,\mathrm{ab}^{-1}$ and after taking into account global constraints on the active-heavy mixing~\cite{Fernandez-Martinez:2016lgt} 
(dot-dashed line in Fig. \ref{fig:lhcSeesaw_ISS}(upper)), the HL-LHC is able to probe heavy neutrino masses up to $350\,\mathrm{GeV}$ 
and mixing down to $\vert V_{\ell N}\vert^2\simeq 10^{-3}$ could be probed.

Another possibility is to search for lepton flavor violating (LFV) final states such as
\begin{equation}
 q ~\overline{q'} \rightarrow  ~N ~\ell^\pm_1 \rightarrow ~\ell^\pm_1 ~\ell^\mp_2 ~ W^\mp  \rightarrow ~\ell^\pm_1 ~\ell^\mp_2 ~j ~j\,,
 \label{eq:LFVfinalstate}
\end{equation}
which was for example studied in the inverse seesaw (ISS), a low-scale variant of the type I, in Ref~\cite{Arganda:2015ija}. 
Due to the strong experimental limits on $\mu\to e\gamma$ by
...
events of this type could be produced for neutrino masses below 700 (1000)
...
%-------------------------------------------------------------------
\begin{figure}[!t]
%\includegraphics[width=0.45\textwidth]{\main/section6FlavorRelatedStudies/img/lhcSeesaw_PhenoType1_lumiVsMNMuMu14TeV_1411_7305.pdf}	\label{fig:seesawIMassReach}	
%\includegraphics[width=0.45\textwidth]{\main/section6FlavorRelatedStudies/img/lhcSeesaw_PhenoType1_sMuMuVsMN14TeV_1411_7305.pdf}	\label{fig:seesawIMixingReach}	
\caption{
Using the LNV final state $\mu^\pm \mu^\pm jj$,
(L) required luminosity for $3~(5) \sigma$ evidence (discovery) as a function of heavy neutrino mass $m_N$ assuming optimistic (brown) 
and pessimistic (purple) mixing scenarios~\cite{Alva:2014gxa};
(R)  sensitivity to $N-\mu$ mixing~\cite{Alva:2014gxa} with the optimistic (pessimistic) mixing scenario is given by the horizontal dashed (full) line.
}
...
the Majorana nature of the heavy neutrinos can leads to striking LNV collider signatures,
...
Assuming the pessimistic mixing scenario, %\pf{pessimistic mixing not defined} 
the HL-LHC with 3 ab$^{-1}$ 
is able to discover a heavy neutrino with a mass of $m_N\simeq400\,\mathrm{GeV}$ 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\paragraph*{Heavy Neutrinos and the Left-Right Symmetric Model at Colliders}\label{sec:lhcSeesawlrsmCollider}

%-------------------------------------------------------------------
\begin{figure}[!t]
\begin{center}
%\includegraphics[scale=1,width=.45\textwidth]{\main/section6FlavorRelatedStudies/img/lhcSeesaw_LRSM_ppWR_XSec_vs_Mass_1607_03504.pdf}  \label{fig:lrsm_HLvsHL_LHC_WRXSec_vs_Mass}		
%\includegraphics[scale=1,width=.45\textwidth]{\main/section6FlavorRelatedStudies/img/lhcSeesaw_LRSM_WR_jN_Disc_vs_Mass_1607_03504.pdf} \label{fig:lrsm_HLvsHL_LHC_WRXDisc_vs_Mass}		
%\\
...
\caption{
Upper:
(L) The total $pp\to W_R$  cross section at NLO+NNLL(Threshold)
and
(R) $5~(2)~\sigma$ discovery potential (sensitivity) via $W_R$ decays to an electron and neutrino jet $(j_N)$,
as a function of $W_R$ mass and at $\sqrt{s} = 14$ and 27 TeV~\cite{Mitra:2016kov}.
%
Lower:
...
The Left-Right Symmetric Model (LRSM) remains one of the best motivated high-energy completions of the SM.
It addresses the origin of both tiny neutrino masses via a Type I+II Seesaw hybrid mechanism 
as well as the SM's $V-A$ chiral structure through the spontaneous breakdown of an SU$(2)_L\otimes$SU$(2)_R$ symmetry, 
amongst other low-energy phenomena.
At collider scales, the model predicts the presence of new heavy gauge bosons that are closely aligned with their gauge states $(W_R^\pm,~Z_R')$,
heavy Majorana neutrinos $(N)$, and a plethora of neutral and electrically charged scalars $(H_i^0,~H_j^\pm,~H_k^{\pm\pm})$.
Unlike U$(1)_{BL}$ neutrino mass models, the LRSM gauge couplings are fixed to the SM Weak coupling constant, up to (small) RG-running corrections.
...
Of the many collider predictions the LRSM, one of its most promising discovery channels is the production of heavy Majorana neutrinos
from resonant $W_R$ currents with $N$ decaying via a lepton number-violating final state. At the partonic level, this is given by~\cite{Keung:1983uu}
\begin{equation}
 q_1 \overline{q_2} \to W_R \to N ~\ell^\pm_i \to \ell_i^\pm \ell_j^\pm W_R^{\mp *} \to \ell_i^\pm \ell_j^\pm q_1' \overline{q_2'}
 \label{eq:seesawLRSMDY}
\end{equation}
and has been extensively studied throughout the literature. For details; see \cite{Cai:2017mow} and references therein.
...
Fig. \ref{fig:lrsm_HLvsHE_LHC}(upper right) show the requisite integrated luminosity for $5~(2)~\sigma$ discovery of 
\eqref{eq:seesawLRSMnujet} at $\sqrt{s}=14$ and 27 TeV.
%
...
\paragraph*{Type II Scalars Discovery Potential at the HL- and HE-LHC}\label{sec:lhcSeesawtype2}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%\includegraphics[scale=1,width=.45\textwidth]{\main/section6FlavorRelatedStudies/img/lhcSeesaw_Type2_L-MH-1tau-LHCX14_1802_0094.jpg} 	\label{fig:LMtau-typeii-LHCX14}
%\\
%\includegraphics[scale=1,width=.45\textwidth]{\main/section6FlavorRelatedStudies/img/lhcSeesaw_Type2_L-MH-1tau-LHCX27_1802_0094.jpg}	\label{fig:LMtau-typeii-LHCX27}
...
Upper (R)- Lower (R):
Requisite luminosity versus $M_{H^{\pm\pm}}$ for $5~(3)\sigma$ discovery (evidence)
for the process $pp\to H^{++}H^{--}\to \tau_h \ell^\pm \ell^\mp \ell^\mp$,
where $\tau^\pm\to \pi^\pm \nu $, 
for the NH and IH at $\sqrt{s}=14,$ 27, and 100 TeV, respectively.
}
\label{fig:lhcSeesawtypeiiScalars}
\end{figure}
%-------------------------------------------------------------------

The Type II Seesaw hypothesizes the existence of a new scalar SU$(2)_L$ triplet that couples to SM leptons in order to 
reproduce the light neutrino mass spectrum and oscillation data.
This is done by the spontaneous generation of a left handed Majorana mass for the light neutrinos.
Moreover, the Type II scenario is notable for the absence of sterile neutrinos, 
demonstrating that light neutrino masses themselves do not imply the existence of additional fermions.
The most appealing production mechanisms at hadron colliders of triplet Higgs bosons are the pair production
...
\end{eqnarray}
followed, in the most general situation, by lepton flavor- and lepton number-violating decays to SM charged leptons.
...
One can discriminate between different heavy scalar mediated neutrino mass mechanisms, such as those found in the Type II Seesaw and Zee-Babu model, 
by studying the distributions of tau leptons' decay products~\cite{Sugiyama:2012yw,Li:2018jns}. 
...
Figs.~\ref{fig:lhcSeesawtypeiiScalars}(upper right)-(lower right) displays
...
$\sqrt{s} = 14$, 27, and 100 TeV, respectively.
For the one $\tau$ channel with $\tau^\pm\to \pi^\pm \overset{(-)}{\nu_\tau}$, 
...
\paragraph*{Type III Leptons Discovery Potential at HL- and HE-LHC}\label{sec:lhcSeesawtype3}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{figure}[!t]
\begin{center}
% \includegraphics[width=0.9\textwidth]{\main/section6FlavorRelatedStudies/img/lhcSeesaw_Feynman_TypeIIIMultiProd_1711_02180.pdf}	\label{fig:type3Seesaw}
% \\
%  \includegraphics[scale=1,width=.48\textwidth]{\main/section6FlavorRelatedStudies/img/lhcSeesaw_Type3_XSec_vs_Mass_1509_05416.pdf} \label{fig:type3_HLvsHL_LHC_XSec_vs_Mass}
%  \includegraphics[scale=1,width=.48\textwidth]{\main/section6FlavorRelatedStudies/img/lhcSeesaw_Type3_Disc_vs_Mass_1509_05416.pdf} \label{fig:lhcSeesawtype3_HLvsHL_LHC_Disc_vs_Mass}
\end{center}
\caption{
Upper:
Born level production of Type III leptons via (a) Drell-Yan, (b) gluon fusion, and (c) photon fusion; from \cite{Cai:2017mow}.
Lower:
  At $\sqrt{s}=14$ and 27 TeV and as a function of heavy triplet lepton mass,
 (L) the summed inclusive production cross section [fb] of $pp\to NE^\pm + E^+E^-$, at NLO in QCD~\cite{Ruiz:2015zca},
 and
 (R) the requisite integrated luminosity [fb$^{-1}$] for 5~(2)~$\sigma$ discovery~(sensitivity) to $N E^\pm + E^+E^-$ 
 based on the analyses of~\cite{Arhrib:2009mz,Li:2009mw}.
}
\label{fig:lhcSeesawtype3}
\end{figure}

Low-scale Type III Seesaws hypothesizes the existence of heavy electrically charged $(E^\pm)$ and neutral $(N)$ leptons, 
which form a triplet under SU$(2)_L$,
that couple to both SM charged and neutral leptons through mixing/Yukawa couplings.
Due to their gauge charges, triplet leptons also couple to EW gauge bosons appreciably 
and do not feature suppressed production cross section typical of Seesaw scenarios with gauge singlet fermions.
The presence of electroweak gauge couplings also implies that once a collider energy and mass are stipulated, 
the triplet lepton 
pair production cross section can be computed, up to small (and potentially negligible) mixing effects.
Production mechanisms commonly found in the literature, and shown in Fig. \ref{fig:lhcSeesawtype3}(upper),
include charged current and neutral current Drell-Yan, photon fusion, and gluon fusion if one considers heavy-light charged lepton associated production.
...
and photon fusion $(\gamma\gamma\to E^+E^-)$, the Drell-Yan processes remain the dominant production channel of triplet leptons when kinematically accessible.
In light of this, in Fig. \ref{fig:lhcSeesawtype3}(lower left), 
the summed cross sections for the Drell-Yan processes,
\begin{equation}
 pp\to \gamma^*/Z^* \to E^+E^- \quad\text{and}\quad 
 pp\to W^{\pm *} \to E^\pm N,
\end{equation}
are shown at NLO in QCD, following \cite{Ruiz:2015zca}, as a function of triplet masses (assuming $m_N = m_E$), at $\sqrt{s}=14$ and 27 TeV.
...
For decays of EW boson to jets or charged lepton pairs, triplet lepton can be full reconstructed from their final-state 
...
\input{section10/High_pT.tex}
\input{section10/experimental.tex}
