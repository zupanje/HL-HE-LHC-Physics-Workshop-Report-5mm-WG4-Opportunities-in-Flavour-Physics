...
Assuming that $S$ is the only light NP state, its couplings to the SM quarks are given by dimension 5 effective operator (not displaying generational indices)
\begin{equation}\label{eq:lag}
 \mathcal{L} = -{\bar{q}_L}{\tilde{Y}}\frac{S}{f}  \tilde{H}{u_R} + \text{h.c.}\supset \tilde{g} \frac{m_t}{f} \bar{t}_L S c_R + \text{h.c.},
\end{equation}
where $f$ is the NP scale, and on r.h.s. we introduced a new flavour violating coupling $\tilde g$. The three Benchmark Points (BP) shown in Figs. \ref{fig:sbb2} 
%and \ref{fig:sgg2} 
are
