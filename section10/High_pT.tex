...
{\it\small Author(s) (TH): Alejandro Celis, Admir Greljo, Lukas Mittnacht, Marco Nardecchia, Tevong You.}
...
%\td{where do we review anomalies, cite here}
% in this context.
...
\caption{Schematic of the LHC bounds on  LQ showing complementarity  in constraining the  $(m_{LQ},y_{q_{\ell}})$ parameters. The three cases are:
 pair production $\sigma \propto y_{q_{\ell}}^0 $ (grey), single production $\sigma \propto y_{q_{\ell}}^2 $ (blue), and Drell-Yan $\sigma \propto y_{q_{\ell}}^4$ (red). From~\cite{Dorsner:2018ynv}.}
\label{fig:LQstrategy}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.40 \textwidth]{section10/img/S3.pdf}~~~~~~~
...
\item {\bf Leptoquark production mechanism:}  The size of the couplings required to explain the $R(D^{*})$ anomaly is typically very large, roughly $y_{q\ell} \approx m_{\rm LQ}/(1 {\rm~TeV})$. Depending of the actual sizes of the leptoquark couplings and its mass we can distinguish three regimes that are relevant for the phenomenology at the LHC:
 	\begin{enumerate}
