...
{\bf Author(s): F. Bishara, R. Harnik,  A. Martin, M. Schlafer, Y. Soreq. E. Stamou (TH); Oscar Augusto De Aguiar Francisco, Lorenzo Sestini (EXP)} (anticipated length 10 pages)
...
In the SM the origin of flavor structure are the Higgs couplings to the fermions. In the SM the  
...
\end{align}
%
while the tree-level flavor changing couplings are zero. 
Currently, only the third generation Yukawa couplings have been measured and found to be in agreement with the SM prediction, see Refs.~\cite{}\td{missing}.
...
Experimentally, we want to test a number of SM predictions: {\em (i)} proportionality, $y_f\propto m_f$; {\em (ii)} the factor of proportionality, $\kappa_{f_i}=1$; {\em (iii)} diagonality (no off-diagonal flavor violation), $\kappa_{{f_i}{f_j}}=0$; {\em (iv)} reality (no CP violation), $\tilde
\kappa_{f_i}=\tilde \kappa_{{f_i}{f_j}}=0$. \td{cite 1605.00433}
...
global fits to all the Higgs data also provide bounds on different Yukawa couplings. In the rest of this section we first briefly discuss the expected deviations in a set of NP models, review current status of constraints, and give the prospects for HL-/HE-LHC. An expanded version of the discussion is available in \td{cite WG2 chapter 7}.
...
%Inclusive Search with Flavor tagging (charm and strange)}
...
%, as well as flavor violating couplings. 
...
%Moreover, 
However, the Higgs flavor violating couplings are strongly constrained by meson mixing~\cite{Blankenburg:2012ex,Harnik:2012pb}, so that the expected rates are too small to be observed. 
