...
The Higgs $p_T$ distribution probes
% of the Higgs boson has been considered before as a probe of high scale 
whether NP particles are running in the $gg\to h +(g)$ loops \cite{Arnesen:2008fb,Biekotter:2016ecg,Brehmer:2015rna,Dawson:2015gka,Schlaffer:2014osa,Grojean:2013nya,Langenegger:2015lra,Bramante:2014hua,Buschmann:2014twa,Azatov:2013xha,Banfi:2013yoa,Buschmann:2014sia}. 
At the same time,  the soft part of the $p_T$ spectrum is an indirect probe of light quark Yukawas~\cite{Soreq:2016rae,Bishara:2016jga}. If 
Higgs is, unlike in the SM, produced from $u \bar u$ or $d \bar d$ fusion,
...
%
Enhanced $s$ or $c$ Yukawa couplings also lead to softer Higgs $p_T$ spectrum. The dominant effect for charm is due to one loop contributions  to $gg\to hj$ process that exhibit double logarithmic enhancement~\cite{Baur:1989cm}, while for strange it is due to $s\bar s$ fusion production of the Higgs~\cite{Soreq:2016rae}. 
%
...
The sensitivities expected at HL-LHC are shown in Fig.~\ref{fig:HiggsDistFuture} (right), assuming total $5\%$ uncertainty in each $p_T$ bin. CMS interpreted the $35.9\,$fb$^{-1}$ measurement of 13\,TeV Higgs $p_T$ spectrum in terms of  95\,\% CL  bounds on $c$ and $b$ Yukawa couplings~\cite{CMS:2018hhg}
...
\end{align}
assuming the branching fractions  depend only on $\kappa_b$ and $\kappa_c$ (are floated freely). 
...
%\end{align}
%%
These bounds on the $c$ Yukawa are weaker\,(stronger) then the bounds from the global fit of the 8\,TeV Higgs data along with the electroweak precision data allowing all Higgs coupling to float~\cite{Perez:2015aoa}. The projected HL-LHC sensitivity is shown in Fig. \ref{fig:HiggsDistFuture} (left).
...
The Higgs boson coupling fits are based a combination of $p_T$ distributions from the $\hgg$~\cite{Sirunyan:2018kta} and 
%$\hzztofourl$
$\hzz^{(*)}\to 4\ell$~\cite{CMS_AN_2016-442} (where $\ell = e$ or $\muon$) decay channels obtained at $\sqrt{s}=13\,$TeV.
% 
  ...
  \begin{center}
   ...
   \includegraphics[width=0.4\linewidth]{\main/section9/plots/projection_kbkc_plot_floatingBRs.pdf}
    ...
    \caption{
        ...
        % 
        For the combination the two standard deviation contour is drawn as a black dashed line, and the negative log-likelihood value on the coloured axis.   As Fig.~\ref{fig:kbkc_couplingdependentBRs}, but with the branching fractions implemented as nuisance parameters with no prior constraint.
        }
