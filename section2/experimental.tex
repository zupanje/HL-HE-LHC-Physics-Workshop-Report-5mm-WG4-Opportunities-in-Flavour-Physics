...
Similarly Fig~\ref{fig:CMSreso} shows the CMS \cite{ CMS-PAS-FTR-18-041} expected HL-LHC performances in the $B^0_s$ mass resolution and proper decay length uncertainty, obtained from a full simulation of the upgraded detector with realistic alignment conditions at the HL-LHC start-up. Besides the improvements in the tracker, which include the L1 trigger capability to reconstruct charged tracks above 2 GeV in transverse momentum with almost offline-like resolutions, the extended pseudorapidity 
...
\caption{Left: HL-LHC and Phase~1 CMS $J/\psi K^+K^-$ invariant mass resolution. Right: Normalized proper decay decay length uncertainty in 2012 data (blue) and HL-LHC Monte Carlo (red) samples}
\label{fig:CMSreso}
\end{figure}
