...
\subsubsubsection{Experimental projections for $b\to c\bar{c}s$ transitions}

Precise measurements of 
%The ATLAS precision in measuring 
$\Gamma_s$ and the weak phase difference $\phi_s$ at the HL-LHC rely on several key factors, among which 
 the most important are the increased statistics, the flavour tagging performance, the B invariant mass and proper-time resolution.
 Improvements in the decay time resolution are expected 
 in ATLAS and CMS thanks to their 
 upgraded inner detectors.
 
 %simulations show an improved decay time resolution, %independent on pile-up conditions. 
Figure~\ref{jpsiphiSensitivity:Mass} shows the ATLAS proper time resolution as a function of the B meson transverse momentum: HL-LHC projections are also compared with the Run 1 and Run 2 performances.

Fig~\ref{} shows the CMS expected Phase-2 performances in the $B^0_s$ mass resolution and proper decay length uncertainty, obtained from a full simulation of the upgraded detector with realistic alignment conditions for the Phase-2 start-up. Besides the improvements in the inner tracker, the extended pseudorapidity 
...
Table \ref{jpsiphitable} summarises the expected sensitivities, compared to the atlas Run 1 measurements and extrapolations to Run 2 statistics. 
...
The CMS study  is based on fully simulated signal events using the signal model $B^0_s \to J/\psi \phi(1020)$ decay as in the previous CMS analysis \cite{CMSPhis}and toy Monte Carlo experiments for some assumed realistic tagging scenarios:$a$ where the flavour tagging performance is based on muons and 
...
\begin{table}
\begin{center}
\caption{CMS statistical uncertainty of $\phi_s$ obtained through MC pseudo-experiments in three different flavour tagging scenarios}
\label{tab:tagging}
...
Figure \ref{jpsiphiSensitivity:Contour} summarises the projected contours in the $\Delta \Gamma_s$ vs $\varphi_s$ plane for the ATLAS-CMS-(LHCB?) different scenarios presented.
...
The ATLAS and CMS Run I analysis are driven by the statistical uncertainty. 
