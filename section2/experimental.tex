...
Similarly Fig~\ref{fig:CMSreso} shows the CMS expected HL-LHC (Phase 2) performances in the $B^0_s$ mass resolution and proper decay length uncertainty, obtained from a full simulation of the upgraded detector with realistic alignment conditions for the Phase-2 start-up. Besides the improvements in the tracker, which include the L1 trigger capability to reconstruct charged tracks above 2 GeV in transverse momentum with almost offline-like resolutions, the extended pseudorapidity 
...
\begin{figure}[t]
\includegraphics[width=0.4\textwidth]{section2/figures/CMSmassplot.png}
\includegraphics[width=0.5\textwidth]{section2/figures/CMSresoPlot.png}
\caption{Left: Phase 2 and Phase 1 CMS $J/\psi K^+K^-$ invariant mass resolution. Right: Normalized proper decay decay length uncertainty in 2012 data (blue) and Phase 2 Monte Carlo (red) samples}
\label{fig:CMSreso}
\end{figure}

\begin{table}[t]
\caption{Summary of the ATLAS $B_s\to J/\psi\phi$ performance at the HL-LHC compared with the Run 1 measurements and projections. Numbers in parenthesis are predictions. $\epsilon$ is the flavour-tagging efficiency, D=1-2$\omega$ is the dilution factor, $\omega$ the wrong tag fraction, and $\sigma(t)$ is the proper time resolution.}
\label{jpsiphitable}
\center
...
Fig.~\ref{fig:CMStag} shows the CMS $\phi_s$ statistical uncertainty distribution in the different tagging scenarios, while Table~\ref{tab:tagging} summarizes the corresponding performances.
\begin{figure}[t]
\begin{center}
\includegraphics[width=0.4\textwidth]{section2/figures/CMS_tagging_power.png}
\caption{Variation of the $\phi_s$ statistical uncertainty as function of the tagging power in different flavour tagging scenarios discussed in the text.}
\label{fig:CMStag}
\end{center}
\end{figure}
Both the ATLAS and CMS systematic uncertainty is expected to be reduced to 1 mrad in the HL-LHC period, and thus the total $\phi_S$  uncertainty will still be statistically limited.

\begin{table}[t]
\begin{center}
\caption{CMS statistical uncertainty of $\phi_s$ obtained through Monte Carlo  pseudo-experiments in three different flavour tagging scenarios, explained in the text. $\epsilon$ is the flavour-tagging efficiency, $\omega$ the wrong tag fraction, and D=1-2$\omega$ is the dilution factor.}
\label{tab:tagging}
\begin{tabular}{lcccc}\hline
tagging scenario & $\epsilon$ [\%] & $\omega$[\%] & $\epsilon D^2$ [\%] & $\delta^{stat}_{\phi_s}$ [mrad] \\ \hline
$a$        & 32  	           & 39.4         &   1.4               & 5.6   		  \\
$b$        & 8  	           & 30.2         &   1.2               & 5.8   		  \\
$c$        & 33  	           & 36.4         &   2.4               & 4.6   		  \\
\hline
...
The expected precision on $\phi_{\squark}^{c\bar{c}s}$ after \upgradetwo\ will be  $\sim 3 \mrad$ from all modes combined.
