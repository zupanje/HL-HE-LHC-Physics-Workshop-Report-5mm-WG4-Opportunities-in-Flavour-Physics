...
\caption{ CMS normalized proper decay decay length uncertainty in 2012 data (blue) and HL-LHC Monte Carlo (red) samples}
\label{fig:CMSreso}
\end{center}
\end{figure}
...
Fig.~\ref{fig:CMStag} shows the CMS $\phi_s$ statistical uncertainty distributions for different tagging scenarios.
Assuming the new tagging power to be in the range 1.2-2.4\% , and a
total of 9 million $B^0_s$  candidates, we expect the $\phi_s$ statistical
...
\caption{Variation of the $\phi_s$ statistical uncertainty in CMS as function of the tagging power in the three different flavour tagging scenarios $a), b)$ and $c)$ discussed in the text, marked in green, blue and red respectively. A fit function $1/\sqrt{t}$, where $t$ is the tagging power (\%), is also shown to describe the $\phi_s$ behaviour in a continuous range of the tagging power from 1.0 to 2.6 \% \td{can we denote a), b), c) in the plot?}}
\label{fig:CMStag}
\end{center}
\end{figure}
