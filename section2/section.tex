...
Extracting information on these parameters from data is a challenge for both experimentalists and theorists. First of all the SM depends on a large set of parameters, masses, mixings, and couplings, which are not predicted within the SM, but rather need to be determined experimentally. An additional difficulty relates to the presence of strong interaction, binding quarks into hadrons. This is responsible for most of the theoretical uncertainties in the determinations of CKM matrix elements. 

The CKMfitter group aims at the goal  of determining the CKM parameters by combining a large set of constraints from flavour physics, using a standard $\chi^2$-like frequentist approach, in addition to a specific (Rfit) scheme to treat theoretical uncertainties~\cite{Charles:2004jd,Charles:2011va,Charles:2015gya}. The set of experimental observables, denoted $\vec {\cal O}_{exp}$, is measured in terms of likelihoods that can be used to build a \chisq-like test statistic:
\begin{equation}
