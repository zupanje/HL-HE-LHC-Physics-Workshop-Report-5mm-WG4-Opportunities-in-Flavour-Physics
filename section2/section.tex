...
\Blu{ It is important to note that the penguin contributions in different $b\to c\bar c s$ channels are different, so that one cannot anymore average $\sin 2\beta$ from $B\to J/\psi K_S, B\to \eta_c K_S,\ldots$, without first correcting for the penguin contributions on a channel by channel basis}. 
