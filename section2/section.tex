...
\subsection{Structure of the CKM matrix and statistical approaches}
...
unitary Cabibbo-Kobayashi-Maskawa (CKM) matrix~\cite{Cabibbo:1963yz,Kobayashi:1973fv}, as 
...
CKM}=V_{u{L}}^\dag V_{d{L}}$ is the unitary CKM matrix:
...
\right).
\end{equation}
The CKM matrix induces flavour-changing transitions inside and between generations in the charged sector at tree level ($W^\pm$ interaction). By contrast, there are no flavour-changing transitions in the neutral sector at tree level.

A strong hierarchy is observed experimentally among the CKM matrix elements: transitions within the same generation are characterised by $V_{\rm CKM}$ elements of $\mathcal{O}(1)$, whereas
...
$V_\text{CKM}$. This
expansion yields the following parametrisation up to
$\mathcal{O}\left(\lambda^{6}\right)$:
...
\right).
\end{equation}
The CKM matrix is complex; thus, \CP violation is allowed if
...
Orthogonality relations can be written involving two columns or two rows of the unitary CKM matrix, and they can be represented as triangles in the complex plane. It is standard to focus on the orthogonality relation:
\begin{equation}
{V_{ud}V_{ub}^{*}}+{V_{cd}V_{cb}^{*}}+{V_{td}V_{tb}^{*}} = 0
\end{equation}
as these products of CKMs are involved in $B$-meson weak decays are of similar $O(\lambda^3)$ size. As show in Fig.~\ref{fig:UTtriangle}, the unitarity triangle corresponds to a rescaling by $V_{cd}V_{cb}^{*}$ of this orthogonality relation, with apices located at $(0,0)$, $(1,0)$ and $(\bar\rho,\bar\eta)$. The angles of the
UT are related to the CKM matrix elements as
\begin{equation}
...
\caption{
The Unitarity triangle. The parameters $\rho$ and $\eta$ are defined as 
$\rho+i\eta=(1-\lambda^2/2+O(\lambda^4)(\bar\rho+i\bar\eta)$.
}
...
Due to its economical structure in terms of only four parameters and
its consequences for \CP violation, the CKM matrix can be
determined through many different quark transitions. These
correspond to $\Delta F = 1$ decays or $\Delta F=2$ processes
related to neutral-meson mixing.
Extracting information on these parameters from data is a challenge for both experimentalists and theorists, since the SM depends on a large set of parameters which are not predicted within its framework, and must be determined experimentally. An additional difficulty stems from the presence of the strong interaction binding quarks into hadrons, which is responsible for most of the theoretical uncertainties discussed when determining the CKM matrix parameters. 

The CKMfitter group aims at this goal by combining a large set of constraints from flavour physics, using a standard $\chi^2$-like frequentist approach, in addition to a specific (Rfit) scheme to treat theoretical uncertainties~\cite{Charles:2004jd,Charles:2011va,Charles:2015gya}. The set of experimental observables, denoted $\vec {\cal O}_{exp}$, is measured in terms of likelihoods that can be used to build a \chisq-like test statistic:
\begin{equation}
...
1/2 page for each constraint

\subsubsection{$|V_{ud}|,|V_{us}|,|V_{cd}|,|V_{cs}|$}
% S. Descotes-Genon

Accurate constraints on the first and second rows and columns of the CKM matrix come from leptonic ($\pi\to e\nu$, $K\to e\nu$, $K\to\mu\nu$, $\tau\to \pi\nu_\tau$,  $\tau\to K\nu_\tau$, $D\to \mu\nu$, $D_s\to \mu\nu$) and semileptonic decays ($K\to\pi e\nu$, $D\to \pi e\nu$, $D_s\to Ke\nu$). The extraction of CKM matrix elements requires the knowledge of hadronic inputs (decay constants for the leptonic decays, normalisation of the form factors at $q^2=0$ for the semileptonic decays) and electromagnetic/isospin corrections when available (i.e. for kaon and pion decays)~\cite{Antonelli:2010yf}.

Another prominent constraint for $|V_{ud}|$ comes from the consideration of the superallowed $\beta$ decays of 20 different nuclei~\cite{Hardy:2014qxa,Towner:2015woa,Hardy:2018zsb} (there are other constraints, but less powerful due to experimental uncertainties and/or theoretical systematics difficult to assess), which provides a very accurate constraint on $|V_{ud}|$.
...
Confidence 
Level (CL)\cite{Charles:2017evz}: 
\begin{equation} \label{eq:alphadir}
\alpha_{\rm dir} = (86.2^{+4.4}_{-4.0} \xspace\cup\xspace 178.4^{+3.9}_{-5.1})^\circ\,,
\end{equation}
where the resolution is so far significantly larger than the theoretical uncertainty related to hypotheses underlying the SU(2) analysis that may affect the  $\alpha_{\rm dir}$ determination by around $2^\circ$. The solution near $90^\circ$ is in  good agreement with the indirect determination obtained by the global fit of the flavour data\cite{CKMfitterwebsite}.\\
%\begin{equation}
...
The experimental data is so far statistically limited for all the considered charmless modes. Due to the limited reconstruction efficiency of the neutral pions and the large associated background in the hadronic environment, the LHC contribution to the isospin analyses is so far confined to the final states reconstructed from charged pions only, i.e the measurement of the branching ratio and the time-dependent $CP$-asymmetries in the  $B^0\to\pi^+\pi^-$ decay\cite{Aaij:2012as,Aaij:2018tfw}, and the branching ratio and fraction of longitudinal polarisation in the colour-suppressed decay mode, $B^0\to\rho^0\rho^0$ \cite{Aaij:2015ria}. A detailed prospective exercice on the CKM angle $\alpha$ has been performed in~\cite{Charles:2017evz} by evaluating the individual impact of the charmless decay observables on the $\alpha$ determination.\\
The global combination of the  decay-specific determinations of $\alpha$ is dominated by the $B\to\rho\rho$ data that provides a constraint with a relative uncertainty at the level of 5\%.  All other observables remaining unchanged, improving the accuracy of the branching ratio of the charged mode $B^+\to\rho^+\rho^0$ would  improve the resolution on $\alpha$ only marginally. Improving the measurements for the neutral modes, in particular the colour-suppressed $B^0\to\rho^0\rho^0$ decay, has a larger impact, essentially driven by the $CP$-asymmetries parameters.\\
...
The extraction of the weak phase $\alpha$ in the the $B^0\to(\rho\pi)^0$ system relies on the analysis of the interference between the intermediate modes $B^0\to\rho^+\pi^-$, $B^0\to\rho^-\pi^+$ and  $B^0\to\rho^0\pi^0$. The Dalitz analysis of the three-pions final states permits a determination free from the discrete ambiguities usually affecting the (quasi) two-body modes, $B\to\rho\rho$ and $B\to\pi\pi$ . At LHCb, such analysis has to face the two fold difficulty of reconstructing the soft $\pi^0$ in the $\rho^+\pi^-$, $\rho^-\pi^+$ interference region and/or to separate the high-energy $\pi^0$ from the single photons from the radiative decays background in the  $\rho^0\pi^0$, $\rho^\pm\pi^\mp$ interference regions. At high luminosity, the analysis may take advantage of the Dalitz decays of the $\pi^0$ or the photon conversion in the detector material.

\subsubsection{Angle $\beta$} 
	First UT angle to be measured, $\beta$ is extracted from the time-dependent CP asymmetry of $b\to c\bar c s$ decays 
	with a charmonium state and a $K$ meson in the final state, which measures Im$\lambda_f=$Im$(q/p \bar{A}_f/A_f)$. The prototype of these decays is $B\to J/\psi K_S$. Given the amplitude decomposition $A_{J/\psi K_S} = V_{cs}V_{cb}^* T + V_{us}V_{ub}^* P$, these decays are dominated by the isoscalar tree-level amplitude $T$, while the subleading term $P$ is both loop and doubly Cabibbo suppressed. Neglecting $P$, the expression of the time-dependent CP asymmetry $a^\mathrm{CP}_t(B\to J/\psi K_S)=\sin2\beta\,\sin(\Delta m_B\, t)$ has a single oscillatory term, the amplitude of which gives experimental access to $\sin2\beta$, providing a determination of the CKM angle with a four-fold ambiguity.~\footnote{This ambiguity can be reduced using measurements sensitive to $\cos2\beta$ such as the time-dependent transversity analysis of $B\to J/\psi K^*$.} This approximation is justified as long as the relative error on $\sin2\beta$ is larger than the correction induced by the subleading term $\sim \lambda^2 P/T$. The present relative uncertainty on $\sin2\beta$ from various charmonium states (including $J/\psi$ / $\psi(2S)$ / $\chi_{c1}$ / $\eta_c$ $K_S$, $J/\psi K_L$, $\psi(nS)K^0$) is about $2.4\%$. Double Cabibbo suppression amounts to $5\%$ and $P/T < 1$ is expected. Yet the precise suppression due to $P/T$ is unknown, likely ranging between some tens and a few percent, the latter estimate given by the perturbative evaluation of the penguing amplitude. Thus, depending on the actual value of $P/T$, the subleading amplitude could be a limiting factor for the extraction of $\sin2\beta$ already now or, in the opposite case, be safely neglected
	...
	With present data, the typical shift in $\beta$ is $0.5$--$1^\circ$ with about $100\%$ uncertainty, dominated by the experimental uncertainties. Thus, the improvement in the measurement the time-dependent asymmetry in $B\to J/\psi K_S$ should be accompanied by a corresponding improvement in the measurement of the control channels in order to keep the subleading amplitude under control and extract $\sin2\beta$ with a subpercent precision.
...
The current best sensitivity is obtained using results of the charged 
$B$-decays into $\tilde{D} K^-$, $\tilde{D}^{*} K^-$, and $\tilde{D} K^{*-}$ modes, where $\tilde{D}$
is a $D^0$ or $\overline{D}^0$ meson decaying to a same final state. The $D^0$ corresponds
to the leading $b \to c$ transition, whereas the $\overline{D}^0$ is produced through 
a Cabibbo- and colour-suppressed $b \to u$ transition. The amplitude ${\cal A}_{b\to c}$ is proportional 
to $\lambda^3$, while the amplitude  ${\cal A}_{b\to u}$ is proportional
to $\lambda^3 \sqrt{\bar{\eta}^2+\bar{\rho}^2}$ $\times e^{i( \delta_B - \gamma)}$.
In these decays, when the $D^0$ or $\overline{D}^0$ decay to a common final state, 
...
and the relative strong phase $\delta_B$ between them.
The value of $r_B$, which determines the size of the direct \CP\ asymmetry,
is predicted~\cite{Gronau:2002mu} to lie in the range 0.05--0.2. The smaller
$r_B$ is, the smaller is the experimental sensitivity to $\gamma$.

Different methods exploit this interference: the Gronau-London-Wyler (GLW)
...
associated to the ${\cal A}_{b\to c}$ amplitude, is required to decay to a doubly
Cabibbo-suppressed decay (DCSD), while the $\overline{D}^0$ meson, associated to the
...
final state such as $K_S^0 h^+ h^-$ (where $h=\pi$ or $K$). Those techniques 
rely on clean theoretical assumptions as the contributions to the amplitudes 
are originated only from tree-level transitions, to an excellent 
approximation~\cite{Brod:2013sga}.
All variations are sensitive to the same $B$ decay parameters and can therefore
be combined in a single fit to extract the common weak phase $\gamma$, as well 
as the hadronic parameters associated to each method.

\subsubsection{Angle $\beta_s$}
	The angle $\beta_s$ belongs to the squeezed UT $\sum_{q_u} V_{q_u b}^*V_{q_u s}=0$ instead of the usual $\sum_{q_u} V_{q_u b}^*V_{q_u d}=0$. Given its reduced sensitivity to $\bar{\rho}$ and $\bar{\eta}$, it is not a strong constraint in the UT plane. On the other hand, the UT analysis provides a very precise indirect determination of its value that can be compared with the direct determination extracted from the time-dependent transversality analysis of $B_s\to\phi K_S$. At present, we have $\beta_s^\mathrm{direct} = 0.60\pm 0.88$ and  $\beta_s^\mathrm{UTA} = 1.06\pm 0.03$. The future uncertainties of the direct and indirect determinations are expected to be $\pm 0.01$ and $\pm 0.007$ respectively, allowing for a deeper investigation of contributions to $B_s$--$\bar{B}_s$ mixing amplitude beyond the SM.
	It is worth noting that, similarly to the $\beta$ case, shifts of few degrees to the value of $\beta_s$ extracted from the time-dependent analysis caused by the subleading amplitude cannot be excluded, producing however a much larger relative uncertainty, as $\beta_s$ itself is doubly Cabibbo suppressed. Indeed, for this reason, some control of doubly Cabibbo suppressed contributions is required to meaningfully extract $\beta_s$ at the SM level.
	Also here, this control can be provided by $SU(3)_f$ related channels, with the additional caveat that $\phi$ is a (almost equal) mixture of $SU(3)_f$ octet and singlet, introducing an additional source of symmetry breaking.
...
In the last years, the improvements in the constraining power of $\varepsilon_K$ have come from new determination of the B-parameter $B_K$ on the lattice. Nowadays, the $B_K$ theoretical error is approaching the percent level and the dominating uncertainty comes from the long-distance contributions, the best estimate of which has an error of $2\%$~\cite{Buras:2010pza} and no clear path of improvement. A breakthrough could come from developing ideas for computing these terms on the lattice, using recent techniques to cope with non-local operators.
...
The mixing of the $B_d$ and $B_s$ mesons can be described upon introducing
the mass and decay matrices, $M^q=M^{q\dagger}$ and
$\Gamma^q=\Gamma^{q\dagger}$. These matrices are involved in the evolution
operator for the quantum-mechanical description of the  $B_q--\overline{B}_q$
...
The prediction of $\Delta m_q$ in the SM is dominated by boxes involving top quarks, and it is given by
	\begin{equation}
	\label{eqof-dmd}
	   \dm_d = \frac{G_F^2}{6 \pi^2}\eta_B m_{B_q}f_{B_q}^2 {\hat{B}}_q
		  m_W^2 S(x_t) \left|V_{tq} V_{tb}^*\right|^2~,
	\end{equation}
where $S$ is the Inami-Lim function~\cite{Inami:1980fz} evaluated at $x_{t}=\overline{m}_t^2/m_W^2$.
and $\eta_B$ are QCD corrections. The value and uncertainty for the perturbative QCD
correction $\eta_{B}$ was originally estimated in Ref.~\cite{Buchalla:1995vs}.
With up-to-date values and uncertainties for $\alpha_{s}$ and the top-quark mass one
obtains $\eta_{B} = 0.5510 \pm 0.0022$~\cite{Lenz:2010gu}.
The prediction involves also non-perturbative inputs, the decay constant $f_{B_q}$ and the so-called bag parameter $\hat{B}_q$ (which is defined by comparison with the vacuum insertion approximation).
...
suffers from significant uncertainties in the theoretical calculation of the
product $f_{B_{d}} \sqrt{B_{B_{d}}}$ of hadronic parameters.
The dependence of the Standard Model prediction for $\Delta m_s$ on
...
$\Delta m_d$. The relevant hadronic
parameters, $f_{B_{s}}$ and $B_{B_{s}}$ as well as $f_{B_s}/f_{B_d}$ and $B_{B_s}/B_{B_d}$,
can be obtained from lattice QCD computation.	


\begin{table}
\caption{Constraints used for the global fit, and the main experimental inputs involved (more information, in particular concerning the theory inputs used, can be found in ref.~\cite{CKMfitterwebsite}. \label{tab:expinputs}}
\begin{center}
...
\subsubsection{Combined contraints on the CKM parameters}\label{sec:2018ckmfit}
...
The results of the UT analysis in the SM from the UFfit collaboration are summarized by the plots in fig.~\ref{fig:UTfigUTplane}. Detailed input values as well as predictions for various CKM-related parameters and observables cab be found in the Summer 2018 update page on the UTfit website~\cite{UTfitwebsite18}.
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=0.5\textwidth]{section2/figures/rhoeta-fullfit-sm.pdf}
...
As discussed above, HL-LHC will naturally provide an improvement in the determination of several flavour observables crucial for the determination of the CKM parameters
We can then consider two phases for the HL-LHC projections: in Phase 1, we assum that the collected data amount for LHCb to 23 fb$^{-1}$ and for CMS/ATLAS to 300 fb$^{-1}$, whereas for Phase 2, we have for LHCb 300 fb$^{-1}$ and for CMS/ATLAS 3000 fb$^{-1}$.
Further observables will be measured at a higher precision thanks to Belle II, and we will assume that the uncertainties reached on these observables correspond to the projections for 50 ab$^{-1}$~\cite{Kou:2018nap}. We also take into account the projected increase in the measurement of $D$ and $D_s$ semi-leptonic decay by BESIII, with an experimental accuracy of 1.2\% on $|V_{cd}|$ and $|V_{cs}|$ combined with the current lattice QCD uncertainties for the decay constant of 0.9\% and 0.8\% respectively)~\cite{BESIII}.
 Since we are
...
The inputs used for the fits are shown in Tab.~\ref{tab:CKMfitterprojinputs}. For an easier comparison with the current situation,
we have added a column ``Current'' corresponding to the current situation concerning uncertainties, but taking central values corresponding to a perfect agreement of the various constraints in the SM. Indeed, the global fit described in Sec.~\ref{sec:2018ckmfit} exhibits slight discrepancies (in particular for $|V_{us}|$) which increase the accuracy of the determination of the CKM parameters. In order to determine the increase in accuracy on the CKM parameters in a fair way, we will therefore compare the three scenarios presented in Tab.~\ref{tab:CKMfitterprojinputs} with the same central values taken to have perfect agreement rather than the results for the Summer 2018 update.

From the global fit, we can determined the 68\% CL intervals for the 4 CKM parameters and other parameters of interest. The uncertainties obtained are given in Tab.~\ref{tab:CKMfitterprojresults} and the corresponding constraints in the $(\bar\rho,\bar\eta$) plane are shown in Fig.~\ref{fig:CKMfitterglobal}. We also show the corresponding constraints in the $B_s$ meson in Fig.~\ref{fig:CKMfitterglobalBs}, and various details of the constraints
...
$\gamma\ [^\circ]$  &  $\pm 5.6$ & $\pm 1.0$ & $\pm 0.35$& Above and Ref.~\cite{Kou:2018nap}\\
$\beta_s\ [{\rm rad}]$ &  $\pm 0.031$ & $\pm 0.014$ & $\pm 0.004$ & Above\\
${\cal B}(B\to\tau\nu)\times 10^4$ &  $\pm 0.21$ & $\pm 0.04$ & $\pm 0.04$ & \cite{Kou:2018nap}\\
${\cal B}(B_s\to\mu\mu)\times 10^9$ &  $\pm 0.66$ & $\pm 0.34$ & $\pm 0.17$ & Above\\
${\cal B}(B_d\to\mu\mu)\times 10^{11}$ &  $-$ & $\pm 3.5$ & $\pm 1.0$ & Above\\
...
The projection of the UT analysis in the HL-LHC era is obtained by performing a global fit using expected values of experimental and theoretical input parameters, as discussed in this Chapter. In particular, lattice uncertainties in 2025 are taken from table~\ref{table:Proj} of section~\ref{sec:lattice}. Given this timescale, we use  experimental uncertainties expected at the end of Run 3 for LHCb and with the full statistics for Belle II. Central values for both theoretical and experimental parameter are taken from a SM point in order to ensure the compatibility of the extrapolated constraints. In table~\ref{tab:utfit_global_inpout} we summarize the extrapolated uncertainties used in the fit. 
\begin{table}
  \begin{center}
	\begin{tabular}{|c|c||c|c||c|c||c|c|}
		\hline 
		 $m_t$ & $0.25$ GeV & $m_b$ & $10$ MeV & $m_c$ & $5$ MeV & $V_{us}$ & $4\times 10^{-4}$\\\hline
		 $|V_{cb}|$ & $4\times 10^{-4}$ & $|V_{ub}|$ & $4\times10^{-5}$ & $|V_{ub}/V_{cb}|$ & $8\times 10^{-4}$ & $\Delta M_d$ & $2\times 10^{-4}$\\\hline
		 $\Delta M_s$ & $6\times 10^{-3}$ & $\sin2\beta$ & $5\times10^{-3}$ & $\alpha$ & $1^\circ$ & $\gamma$ & $1^\circ$\\\hline
 		 $A_\mathrm{SL}^d$ & $5\times 10^{-4}$ & $A_\mathrm{SL}^s$ & $7\times 10^{-4}$ & $\phi_{B_s}$  & $0.01$ & $B_K$ & $5\times10^{-3}$\\\hline
 		 $F_{B_s}$ & $1\times10^{-3}$ & $F_{B_s}/F_{B_d}$ & $6\times10^{-3}$
 		 & $B_s/B_d$ & $5\times10^{-3}$ & $B_s$ & $7\times10^{-3}$ \\\hline
 		 
		 \hline
	\end{tabular} 
\end{center}
\caption{Extrapolated input parameter uncertainties used in the UTfit analysis.} 
...
are shown next to each other zooming on the SM preferred region.
...
\caption{Present (left) and future (right) constraints on the UT plane.}
\label{fig:UTfitFut}
\end{center}
\end{figure}
...
\begin{table}
  \begin{center}
	\begin{tabular}{|c|c|c|c|c|c|c|c|}
		\hline 
		 & $\rho/\bar{\rho}$ & $\eta/\bar{\eta}$ & $A$ &  $\sin2\beta$ & $\gamma$ & $\alpha$ & $\beta_s$ \\ \hline
		present & 9\% & 3\% & 1.5\% & 4.5\% & 3\% & 2.5\% & 3\% \\ \hline
		future  & 2\% & 0.7\% & 0.6\% & 0.6\% & 0.5\% & 0.7\% & 0.7\%\\ \hline
	\end{tabular} 
\end{center}
\caption{Relative uncertainties of UT parameters and angles with current and extrapolated input values for measurements and theoretical parameters.} 
...
determination of the UT shown in Fig.~\ref{fig:UTtree}. This allows us
...
constraints on CP violation in $D$ mixing from Sec. \textbf{refer to
