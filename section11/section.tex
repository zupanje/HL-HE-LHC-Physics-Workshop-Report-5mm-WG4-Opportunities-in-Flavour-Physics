...
{\bf Author(s): Elvira G\'amiz, Michele Della Morte, Enrico Lunghi} (5pages)
...
\item In the fourth column we quote the error corresponding to the published value in the second column and,
  in parenthesis, the error coming from a couple of recent calculations, significantly reducing current errors, but that either are not published yet ($f_+(0)^{K^0\pi^-}$) or are not included in the FLAG-2016 averages (decay constants). The latest will be included in the next release of the FLAG averages in 2019.
...
\begin{tabular}{ccccc}
\hline\hline
Quantity & Published value & Reference & error (to be published/not in FLAG-2016)
& 2025\\
\hline
$f_{K^\pm}$ & $155.7(7)$~MeV & $N_f=2+1$~\cite{Aoki:2016frl} & 0.4\%  &  \\
$f_+^{K\to\pi}(0)$ & $0.9706(27)$ & $N_f=2+1+1$~\cite{Aoki:2016frl} & 0.28\% (0.20\%~\cite{Bazavov:2018kjg}) & 0.12\%\\
...
\begin{tabular}{c}$B\to D^*$ for $|V_{cb}|_{{\rm theor}}$\\$h_{A_1}^{B\to D^*}(\omega=1)$\\\end{tabular} &  & $N_f=2+1$~\cite{Aoki:2016frl} & 1.4\% & 0.4\%(0.7\%)\\  
...
\caption{Current estimates and projections for lattice QCD determinations of hadronic inputs. In the fourth column we quote the error corresponding to the published value in the second column and,
  in parenthesis, the error coming calculations that either are not published yet or are not included in the FLAG-2016 averages. See text for detailed explanations.}
\label{table:Proj}
\end{sidewaystable}
\end{center}
