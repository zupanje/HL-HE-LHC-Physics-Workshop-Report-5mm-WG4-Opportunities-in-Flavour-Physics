...
{\bf Author(s): Elvira Gamiz, Michele Della Morte, Enrico Lunghi} (5pages)
...
and reviewing the main sources of uncertainty for the different quantities.
In general, a naive application of Moore's law (computing power doubling 
each two years), which would give a reduction in the errors
by a factor around 3 by 2025, is not always appropriate as the dominant 
...
an almost up-to-date picture of the precision reached nowadays.
From there it emerges that, for example, the 
%{\it target}
 target accuracy on
...
concerning some subtleties arising in a straightforward application of the
...
is probably going to be the scale setting or, in other words, the conversion from
lattice units to MeV. As a remark, while this is quite obvious for dimensionful
quantities, it is also critical for some dimensionless ones, e.g., 
the hadronic contribution to the anomalous magnetic moment of the muon 
(see~\cite{DellaMorte:2017dyu} for a detailed discussion of this point),
where one needs to convert the muon mass to lattice units.
The current precision in the knowledge of the lattice spacing
is at the half-percent level and going beyond that is challenging, as one needs
a quantity which is both very well known experimentally and precisely computable
on the lattice, concerning statistical as well as systematic uncertainties.
...
Ratios of decay constants can however be very precisely obtained from lattice simulations,
...
$f_{K^+}/f_{\pi^+}$ has reached the 0.15\% error~\cite{Bazavov:2017lyh}.
% 
%{\color{red}
%Anything about $f_K$?
%}

Turning now to the bag parameter $B_K$, relevant for the theoretical prediction
...
one needs either a higher order computation~\footnote{See {\tt https://indico.fnal.gov/event/15949/session/9/contribution/23/material/slides/0.pdf}
...
The FLAG averages of the $B_K$ parameter have been very stable through the different reviews, the main reason being,
...
% assuming that other
%collaborations will produce precise numbers for this quantity 
%(the FNAL/MILC ccollaboration is considering 
%including $B_K$ in the project aiming at computing the $B$ bag parameters with sub-percent precision)
%. 

Concerning the phenomenological implications of $B_K$ however, the main hadronic
...
and preliminary results have been reported at the last Lattice conferences, the most recent update
being in~\cite{Bai:2016gzv}. Although the parameters in the lattice simulations are not physical,
...
Finally, it has been 
%recently 
emphasized in~\cite{Bailey:2018feb} that the theoretical 
estimate of $\varepsilon_K$
based on lattice inputs strongly depends on the value of $V_{cb}$. Its current uncertainty, due
to the tension between inclusive and exclusive determinations, represents then the largest source
of systematic error. 

%\begin{itemize}
%\item BK
%\item FLAG value, 1.3\% not obvious how to get to 0.5\%. However the FLAG number is dominated by one
%result (BMW) from 2011, so a factor two is not unconceivable if other collaborations 
%could produce in the future similar numbers (ETMC is working on that, FNAL ?)

%\item main source of error: not statistics, rather the conversion from lattice scheme
%(SF or RIMOM) to MSbar or the running in the lattice scheme up to the RGI quantity.
%Currently done typically at one-loop. Either one computes the two loop  or
%performs the NP running up to higher scales. In both cases it is going to be somebody
%withn lattice doing the computation.

%\item main issue connecting to epsilonK: long distance contributions. Methodology exists
%to do that on the lattice. A few results appearing ... effect of 8(5)\%

%\end{itemize}

%{\color{red}
%Lattice calculations of light-quark matrix elements have achieved an extraordinary level
%of precision. As an example, the last calculations of the ratio of decay constants
%$f_{K^+}/f_{\pi^+}$ has reached the 0.15\% error~\cite{Bazavov:2017lyh}.
%\begin{itemize}
%\item Add a sentence about why light quarks are easier?
%\item Anything about $f_K$?
%\end{itemize}
%  }
...
depend again upon hadronic matrix elements of local four-fermion operators. 
Historically, those matrix elements have been parametrized in terms of the
so-called bag parameters. In the SM, the mass difference for a
$B_q^0$ meson depends upon a single matrix element, which is proportional to
$f_{B_q}^2\hat B_{B_q}$, or, equivalently, upon a single bag parameter, $\hat B_{B_q}$. 

The progress on the lattice determination of those matrix elements have been
steady in the last decade, with errors $\sim 4-5\%$ for $\hat B_{B_q}$~\cite{Aoki:2016frl}, but
...
reduce the heavy-quark discretization uncertainty and will also allow most precise
...
elements. The recent (not accounted for yet) and projected progress on the
determination of these decay constants will thus contribute to the reduction
in the bag parameters uncertainty. 
...
can be achieved for the bag parameters describing the mixing BSM. 
...
%{\color{red}The following paragraph can be removed}
...
Moving now to momentum-dependent hadronic parameters, we review the status and the prospects for lattice computations
...
is among the most accurately quantities obtained on the lattice. 
The most recent calculation of this form factor, not included in the last
FLAG review, has reduced the error to 0.19\%~\cite{Bazavov:2018kjg}, the
%\footnote{Preliminary but nearly final results has been presented in https://indico.fnal.gov/event/15949/session/3/contribution/294}
same level
as the experimental error~\cite{Moulson:2017ive}. 
The main source of uncertainty on the lattice side is
now statistics, that could be reduced by several lattice collaborations, both with
...
which will further reduce the total uncertainty. Another important sources of error
...
on the QCD form factor, but also that from the electromagnetic and strong isospin-breaking
corrections contributing to the experimental uncertainty.

Lattice calculations are not limited to $f_+^{K^0\pi^-}(q^2=0)$, but there exist results
...
energy dependence usually adopted in experiment, is still less precise using lattice data.
In the future, however, with the improvements discussed above, lattice could provide
...
We come now to a discussion of three groups of $b$-hadron form factors that are of crucial phenomenological importance: heavy meson to stable pseudoscalar meson ($B\to (\pi,K,D)$ and $B_s \to (K, D_s)$), heavy meson to unstable vector meson ($B\to (D^*, K^*))$) and heavy baryon to baryon ($\Lambda_b \to (p, \Lambda_c)$).
Form factors in the first group have been calculated by several collaborations because of the presence of a stable final state particles simplifies the analysis. Form factors in the second group are complicated by the presence of an unstable final state meson; while there is a solid theoretical foundation to treat such situation on the lattice~\cite{Briceno:2014uqa}, it should be noted that the huge hierarchy $\Gamma_{D^*}/M_{D^*} \sim 4 \times 10^{-5} \ll \Gamma_{K^*}/M_{K^*} \sim 6\times  10^{-2}$ reduces the impact of the final state meson decay for the $D^*$ case. 
%Finally, form factors involving baryons are
In all cases, lattice calculations have higher accuracy for kinematical configurations in which the final state hadron has low recoil (i.e. large $q^2$) while experimental measurements tend to have better efficiency at large recoil (i.e. small $q^2$). 

Moreover, it is important to stress the role of form factors parametrizations which must be used to extrapolate the lattice results at low-$q^2$ (as for the $B\to D^{(*)}$ form factors) or when combining lattice and experimental results in order to extract CKM parameters (e.g. $V_{cb}$ and $V_{ub}$). 
...
%
%In general, we expect the impact of different parametrizations to be under control in situations in which both lattice and experiment present differential (binned) distributions which, as explained above, tend to cover different parts of the spectrum, thus allowing for a model independent extraction of the form factor shape. 
%
...
As far as the state of art is concerned, we consider the averages presented in the latest FLAG review~\cite{Aoki:2016frl}. In the following we present estimates of the form factors theoretical accuracy that can be achieved over the next eight years and their impact on the extraction of $V_{ub}$ and $V_{cb}$. As a general rule, we estimate the overall drop in uncertainty to be a factor of 3 both in its statistic and systematic components. 
Thus, for many calculations the total extrapolated lattice uncertainty drops below the 1\% level at which point QED effects need to be included. This can be done on the lattice either in the quenched QED limit or by explicit generating mixed QCD-QED configurations. In particular, the inclusion of QED corrections to form factors is complicated  by IR safety issues which can be nonetheless addressed in lattice calculations~\cite{Carrasco:2015xwa}. It is likely but not guaranteed that QED corrections to the form factors we discuss below will be calculated in the time frame we consider. Therefore, we consider two scenarios according to whether QED corrections have or have not been calculated; in the latter case we add in quadrature an extra 1\% uncertainty. 
\vspace{-0.35cm}
\begin{itemize}
\item
{ $B\to\pi$ and $V_{ub}^{}$.}
The overall point-by-point uncertainty should reduce to the sub-percent level due the adoption of HISQ for heavy quarks on the finer lattices and physical light quark masses. Currently the uncertainty on the extraction of $V_{ub}$ from a simultaneous fit of lattice form factors~\cite{Lattice:2015tia, Flynn:2015mha} and binned measurements of the semileptonic branching ratio is 3.7\%. The theoretical and experimental contributions to this error can be estimated at the 2.9\% and 2.3\% level, respectively. A reduction on the lattice uncertainties along the lines mentioned above would naively reduce the former uncertainty to about 1\%. As a cautionary note we point out that these estimates are sensitive to information on the shape of the form factor which is controlled by the correlation in the synthetic data and by the accuracy of future experimental results. In conclusion, the theory uncertainty on $V_{ub}$ is expected to decrease from 2.9\% to 1\% (1.4\% if QED corrections are not calculated).
%
\item
{ $B\to K$.}
...
presented a calculation of the ratio $f_+^{(B->K)} (0) / f_+^{(Bs->Ds)} (0)$ with a total uncertainty of 13\%,
...
\vspace{-0.3cm}
\begin{itemize}  
\item In the second and third columns we quote the current published best values with references.
\item In the fourth column we quote the error corresponding to the published value in the second column and,
  in parenthesis, the error coming from a couple of most recent calculations that are not published yet but
  significantly reduced the errors.
\item Significant reduction of current (to-be-published) errors in the decay constants are very
...
\begin{tabular}{ccccc}
\hline\hline
Quantity & Published value & Reference & error (to be published)
& 2025\\
\hline
$f_K$ & $155.7\pm0.7$~MeV & $N_f=2+1$~\cite{Aoki:2016frl} & 0.4\%  &  \\
$f_+^{K\to\pi}(0)$ & $0.9706(27)$ & $N_f=2+1+1$~\cite{Aoki:2016frl} & 0.28\% (0.19\%~\cite{KtopiFNALMILC}) & 0.12\%\\
$B_K$  & $0.7625(97)$ &  $N_f=2+1$~\cite{Aoki:2016frl} & 1.3\% & 0.7\% \\
$f_{B_s}$ & 228.4(3.7) &  $N_f=2+1$~\cite{Aoki:2016frl} & 1.6\%(0.56\%~\cite{Bazavov:2017lyh}) & \\
$f_{B_s}/f_{B^+}$ & 1.205(7)& $N_f=2+1+1$~\cite{Aoki:2016frl} & 0.6\%(0.4\%~\cite{Bazavov:2017lyh}) & \\
$B_{B_s}$ & 1.32(5)/1.35(6) & $N_f=2/N_f=2+1$~\cite{Aoki:2016frl} & $\sim 4$\% & 0.8\% \\
$B_{B_s}/B_{B_d}$ & 1.007(21)/1.032(28) & $N_f=2/N_f=2+1$~\cite{Aoki:2016frl} & 2.1\%/2.7\% & 0.5\%\\
$\xi$ & 1.206(17) & $N_f=2+1$~\cite{Aoki:2016frl} & 1.4\% & 0.3\%\\
$\overline m_c (\overline m_c)$ & 1.275(8)~GeV &  $N_f=2+1$ ~\cite{Aoki:2016frl} & 0.6\% & 0.4\%\\
\hline
$f_{K^\pm}/f_{\pi^{\pm}}$ & 1.193(3) &  $N_f=2+1+1$~\cite{Aoki:2016frl} & 0.25\%(0.15\%, symmet.~\cite{Bazavov:2017lyh})&  \\
$f_{D_s}$ & 248.83(1.27) &  $N_f=2+1+1$~\cite{Aoki:2016frl} & 0.5\%(0.16\%~\cite{Bazavov:2017lyh}) & \\
$f_{D_s}/f_{D^+}$ & 1.1716(32) &  $N_f=2+1+1$~\cite{Aoki:2016frl} & 0.27\%(0.14\%~\cite{Bazavov:2017lyh}) & \\
\hline
$B\to \pi$ for $|V_{ub}|_{{\rm theor}}$ &  & $N_f=2+1$~\cite{Aoki:2016frl} & 2.9\% & 1\%(1.4\%)\\
$B\to D$ for $|V_{cb}|_{{\rm theor}}$ &  & $N_f=2+1$~\cite{Aoki:2016frl} & 1.4\% & 0.3\%(1\%)\\
%\begin{tabular}{c}$f_{+,0}^{B\to D}(\omega=1.16)$\\(repres. $\omega$)\\\end{tabular}
(first param. BCL z-exp.) &  & $N_f=2+1$~\cite{Aoki:2016frl} & 1.5\% & 0.5\%(1.1\%)\\
$B\to D^*$ for $|V_{cb}|_{{\rm theor}}$ & - & $N_f=2+1$~\cite{Aoki:2016frl} & 1.4\% & 0.4\%(0.7\%)\\  
$h_{A_1}^{B\to D^*}(\omega=1)$ &  &   & = & =\\
$P_1^{B\to D^*}(\omega=1)$ & & No LQCD available & & 1-1.5\% \\
\begin{tabular}{c}$\Lambda_b\to p(\Lambda_c)$\\for $|V_{ub}/V_{cb}|_{{\rm theor}}$\\\end{tabular} & &
\cite{Detmold:2015aaa}  & 4.9\% & 1.2\%(1.6\%) \\
\hline
$B\to K$ &  & $N_f=2+1$~\cite{Aoki:2016frl} & 2\% & 0.7\%(1.2\%)\\
(first param. BCL z-exp.) & & & & \\ 
$B_s\to K$ &  & $N_f=2+1$~\cite{Aoki:2016frl} & 4\% & 1.3\%(1.7\%)\\
(first param. BCL z-exp.) & & & & \\ 
\hline
\hline
\end{tabular}
...
%\end{small}
%\vspace{-0.1cm}
\caption{Current estimates and projections for lattice QCD determinations of hadronic inputs. See text for detailed explanations.}
\label{table:Proj}
%\end{table}
\end{sidewaystable}
\end{center}
