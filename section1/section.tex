...
As a community we are now in a strikingly different position than we were a decade ago, before the Large Hadron Collider (LHC)  turned on. 
...
agreement with the Standard Model (SM). Coupled with the lack of direct signals of beyond the standard model (BSM) 
...
indirect new physics (NP) searches via precision studies of low-energy flavor-changing observables.
...
a brief introduction to flavor physics, starting with the ``flavor problem'', and the general discussion of probing BSM through flavor transitions. 

\smallskip
{\bf The flavor problem.~~} 
Flavor is the label generically used to differentiate the 12 fermions which, according to the SM, 
...
%  ZL: How could I not... ;)
is one of the big open questions in fundamental physics, often referred to as the ``SM flavor puzzle''. 
...
suggests a possible connection between the flavor puzzle and the electroweak hierarchy puzzle, 
another big open question pointing toward some form of new physics.

\smallskip
{\bf Model-independent considerations.~~}
The above puzzling aspects make flavor physics, i.e., the precision study of flavor-changing processes in the quark and lepton sector, 
a very interesting window on possible physics beyond the SM.  We do not know if there is an energy scale at which the observed flavor structures assume a simpler form. That is, we do not know if 
 the masses and mixing angles, as observed at low energies, can be predicted in terms of a reduced number of 
more fundamental parameters in a theory valid at some high scale.  On the other hand, precision measurements of flavor-changing transitions may probe such scales, 
...
Under such an assumption of heavy NP, the amplitudes describing a flavor changing transition 
...
as is the case for flavor-changing neutral-current 
...
quark masses, hadronic form factors, etc.). For such cases we can expect significant increase in precision with higher 
...
flavor physics program, where the focus is not only on rare or $CP$ violating processes, ``most likely'' affected by NP,
but also on core SM measurements, which help to reduce the theoretical uncertainties. 

\smallskip
{\bf Current anomalies and historical comments.}
Due to the generic sensitivity to high scales, flavor physics has historically played a major role in developing and understanding 
the Standard Model. Flavor physics measurements signaled the presence of ``new'' particles 
...
The great improvements in precision for several flavor-changing process achieved in the last 20 years, both at experimental and theoretical level, represents a very important advancement of the field. 
...
nontrivial flavor structure, able to mimic the strong suppression of flavor-changing neutral-current transitions in the SM.
...
flavor physics. The anomalies recently observed in semileptonic $B$ decays clearly demonstrated a genuine discovery potential, whether their significance increase or decrease with improved measurements.
...
More precise measurements of some of these observables, in particular the lepton flavor universality violating ratios
...
they have clearly exemplified the discovery potential of flavor-physics observables, 
and enlarged our horizon regarding possible BSM scenarios. Before the appearance of these anomalies, 
lepton flavor universality (LFU) was an implicit assumption adopted by the vast majority of proposed BSM scenarios.
...
indirect NP searches from flavor observables and direct searches at high-$p_T$. Interestingly enough, even in the low-scale models 
...
\smallskip
{\bf Connections to lepton flavor violation.~~}
The charged lepton flavor violating (CLFV) processes, such as $\tau \to 3 \mu$, 
are essential parts of the flavor-physics program.  
...
Any attempt to solve the flavor puzzle with new dynamics not far from the TeV scale, such that  the observed hierarchies in the Yukawa couplings are accounted for by the new dynamics, 
...
that violate both quark and lepton flavor, such as $B \to K \tau \mu$.  
...
in a large matrix, with the row and column indices determined by lepton and quark flavors, which is largely unexplored at present. For any NP model that may populate entries in this matrix, there is a large  
%and exhibits the 
...
\smallskip
{\bf Connections with the hierarchy problem and complementarity with high-$p_T$ searches.~~ } 
BSM models proposed to address the electroweak hierarchy problem, such as supersymmetric models or composite models, predict new particles around the TeV scale. For all these models, flavor physics imposes very stringent bounds, requiring a flavor structure not far from that in the SM. This was the main rationale underlying 
the hypothesis of Minimal Flavor Violation (MFV). The MFV hypothesis is an ansatz for the flavor structure of NP that assumes that the SM Yukawa couplings are the only sources of flavor non-degeneracy even beyond the SM.
...
This example illustrates nicely the importance of flavor physics in reconstructing the structure of any NP model addressing the electroweak hierarchy problem, but also its complementarity with the high-$p_T$ experiments, where improved direct bounds relax the flavor structure requirements.

If there are new particles which couple to the SM quarks or leptons,  then, in general, there are corresponding new flavor parameters. Measuring them would be very important in order to understand the structure of new physics.  This has been studied in great detail in the context of SUSY (alignment mechanism
of the soft-breaking terms), and in composite models 
(partial-composite mechanism).
In the specific case of low energy supersymmetry, the squark and slepton couplings may yield measurable effects in FCNC processes and $CP$ violating observables, and may give rise to detectable CLFV transitions.  Observable $CP$ violation is also possible in neutral currents and in electric dipole moments, for which the SM predictions are below the near future experimental sensitivities. The supersymmetric flavor problems, namely the 
observation that TeV-scale SUSY models with generic parameters are excluded by FCNC and $CP$ violation measurements, can be alleviated in several scenarios: (i) universal squark masses (e.g., gauge mediation); (ii) quark--squark alignment, (e.g., horizontal symmetry); (iii) very heavy squarks (e.g., split SUSY). All viable models incorporate some of these ingredients. Conversely, if SUSY is discovered, mapping out its flavor structure, with the help of future more precise flavor tests,
may help answer questions about even higher scales, the mechanism of SUSY breaking, how it is communicated to the MSSM, etc.

\smallskip
\td{need to decide where this goes} {\bf Experimental considerations and the breadth of flavor physics.~~} 
...
\smallskip
{\bf Unexpected discoveries.~~}
It goes without saying that it is impossible to predict truly unexpected future discoveries.  However, it cannot be emphasized enough that the large increase in data sets has the potential to revolutionize the field by unexpected discoveries, and trigger entirely new areas of experimentation.  It should be obvious that exact and approximate conservation laws should be tested as precisely as possible, especially when the experimental sensitivity can substantially increase.  Recall that the discovery of $CP$ violation itself was unexpected, in an experiment whose primary goal was checking an anomalous kaon regeneration result.  New particles with surprising properties were in fact discovered at each of BaBar, Belle, and LHCb; respectively, the discoveries of the $D_{sJ}(2317)$ meson with a mass much below expectations, the discovery of the unexpectedly narrow charmonium-like state $X(3870)$, and the discovery of pentaquarks.  Thus, beside the``classical'' searches for flavor-violating processes mentioned so far, both in quark and lepton sectors, other searches such as the searches for dark sectors in many channels, or not yet conceived BSM searches, will all form important parts of the flavor physics program in the HL-LHC era.

\smallskip
{\bf Theory improvements.~~} 
In the past, large increases in available data always triggered new theory developments, and better understanding of the domain of applicability and accuracy of existing theoretical tools.  It can be anticipated that these fruitful cross-fertilizations will continue to occur in the HL-LHC era between flavor physics experiments and theory.  While there is a substantial suite of measurements whose interpretations will not be limited by hadronic uncertainties, the experimental program can still benefit a lot from theoretical improvements.  For many observables, anticipated lattice QCD improvements are important.  The anticipated improvements in experimental precision also pose interesting challenges for lattice QCD, to robustly address isospin violating and electromagnetic effects in flavor observables.  For many nonleptonic decays, relevant for $CP$ violation, lattice QCD is unlikely to make a big impact. Nevertheless, developing new methods based on effective theories and testing existing approaches can be expected to improve the theoretical understanding of many observables, further enhancing the sensitivity of the experimental program to possible BSM phenomena.

\smallskip
{\bf The main points.~~} To conclude, we summarize the main points highlighted above, 
and which will be expanded upon in the subsequent sections:
\vspace{-\topsep}
\begin{itemize}
\item $CP$ violation and FCNCs are sensitive probes of short-distance 
physics, within \& beyond the SM;
\item Flavor physics probes scales $\gg 1$\,TeV, the  
sensitivity is often limited by statistics not by theory;
\item For most FCNC processes NP /SM $\gtrsim 20\%$ is still allowed, so there is plenty of discovery potential;
\item Of the several tensions between flavor physics data and the SM, some may soon become decisive;
\item Precision tests of SM flavor sector will improve by orders of magnitudes (including CLFV);
\item There are many challenging theory problems, relevant for improving experimental sensitivity;
\item Flavor physics will teach us a lot about physics at shorter distances, complementary to the high-$p_T$ physics program, whether NP is seen or not, and could point to the next energy scale to explore.
\end{itemize}
\vspace{-\topsep}
...
\subsection{Key experimental capabilities at ATLAS and CMS}
...
The ability of general purpose detectors to make precision heavy flavour measurements has been clearly demonstated
%definitely proven 
...
However, for ATLAS and CMS experiments it is not trivial to fully exploit this potential: $B$ physics is expected to be 
among the areas that may suffer most heavily from the HL-LHC conditions. This is a consequence of the relative 
low momenta of typical heavy flavour signatures, and the high high pile-up which might affect the precision of the measurements.
On the other hand, some of the projected Phase-II upgrades of the general purpose detectors, such as the new inner tracker, the improvements of the muon system, the topological trigger capabilities and the possibility to use tracking in the early stages of the trigger chain, promise good detection potential, good pile-up mitigation and even -- in some cases -- and improved performance.
 
The expected high integrated luminosity will allow ATLAS and CMS to study some rare processes at the precision never attained before. The capabilities of excellent tracking and muon detection performances are highlighted by a number of  benchmark channels, $B_{s,d} \to \mu\mu$, $B^0 \to K^{*0} \mu\mu$, $B_s \to J/\psi \phi$, and $\tau \to 3\mu$, that are used for projections. 
...
\clearpage
\subsection{Key experimental capabilities at LHCb}
...
\begin{enumerate}
\item
There is a host of measurements of {\it theoretically clean} observables, such as the \CP-violating phase $\gamma$, the lepton-universality ratios $R_K$, $R_{K^\ast}$ {\it etc.}, or the ratio of branching fractions $R\equiv {\cal{B}} (\Bd \rightarrow \mu^+ \mu^-)$/${\cal{B}} (\Bs \rightarrow \mu^+ \mu^-)$, where knowledge will still be statistically limited after Upgrade I.  The same conclusion applies for other observables such as $\phi_s$ and $\sin 2 \beta$, where strategies exist to monitor and control possible Penguin pollution. The HL-LHC and the capabilities of LHCb Upgrade II offer a unique opportunity to take another stride forward in precision for these quantities.  Advances in lattice-QCD calculations will also motivate better measurements of other critical observables, {\it e.g.} $|\Vub|/|\Vcb|$.  
...
\item
It will be essential to {\it widen the set of observables under study} beyond those accessible at the current experiment and at Upgrade I, {\it e.g.} by including additional important measurements involving  $b \to s/d \ellell$ and $b \to c \ell^-\bar{\nu_l}$ decays.  Improving our knowledge of the flavour sector both through better measurements and through new observables will be essential in searching for, and then characterising, New Physics in the HL-LHC era.
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=0.48\textwidth]{section1/figs/UT_LHCb_now.pdf}
\includegraphics[width=0.48\textwidth]{section1/figs/UT_LHCb_23.pdf}
\includegraphics[width=0.48\textwidth]{section1/figs/UT_LHCb_300.pdf}
\caption{\small  Evolving constraints in the $\bar{\rho}-\bar{\eta}$ plane from LHCb measurements and lattice QCD calculations, alone, with current inputs (2018), and the anticipated improvements from the data accumulated by 2025  (23\,fb$^{-1}$) and 2035 (300\,fb$^{-1}$). More information on the fits may be found in~\cite{}.
 }
