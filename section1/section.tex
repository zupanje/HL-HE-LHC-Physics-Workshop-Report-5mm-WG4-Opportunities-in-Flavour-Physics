...
\smallskip
{\bf Nonperturbative QCD and its role in flavour physics.} A particular class of theoretical uncertainties are those produced by our incomplete understanding of the dynamics of QCD at low energies. Indeed, flavour transitions are mostly measured in weak decays of hadrons and information on nonperturbative functions such as form factors are needed to extract information on the short-distance physics from the experimental data. The last decade has also witnessed important theoretical progress in the calculation of these hadronic parameters, with refinements within the effective-field-theory (EFTs) approaches exploiting e.g. heavy-quark and/or low-energy perturbative expansions and, specially, with major steps given in the field of lattice QCD. For instance, the theoretical precision in the calculation of nonperturbative quantities which are crucial e.g. for the extraction of the CKM matrix element $|V_{us}|$ or for the interpretation of FCNCs such as $B\to\mu\mu$ have reached the per-mille level. On the other hand, most of weak hadron transitions (e.g. non-leptonic decays) involve hadronic matrix elements which are still not accessible to lattice calculations. The understanding of these quantities will also benefit from the breadth of the experimental flavor physics program in the HL-LHC as many can be interconnected by using theoretical methods that exploits symmetries (such as EFTs) or dispersive techniques. 

A particular program which is shedding light (or darkness?) into the dynamics of nonperturbative QCD is Spectroscopy. A plethora of exotic hadronic states, many of which were unexpected or showed intriguing features, have been discovered at the $B$-factories and the LHC. The increase of data samples at the HL-LHC will allow to discover many more of these    
...
\smallskip
{\bf Connections to lepton flavour violation.~~}
...
\smallskip
\td{need to decide where this goes} {\bf Experimental considerations and the breadth of flavour physics.~~} 
...
However, for ATLAS and CMS it is not trivial to fully exploit this potential: $B$ physics is expected to be 
