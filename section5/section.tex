...
Figure~\ref{fig:cms_Tau3Mu} presents the trimuon mass distribution for signal and background in the event category with at least one muon reconstructed by the new triple GEM detectors, which  will be installed in the first station of the upgraded muon system. 
...
The absence of tails in the signal distribution shows that, despite the high pileup (PU=200), muons from $\tau \to 3\mu$ decays are picked correctly.
The projected exclusion sensitivity in the absence of a signal is $\mathcal{B}( \tau \to 3 \mu$) < 3.7 $\times 10^{-9}$  at 90\% CL,while the expected 5 $\sigma$-observation sensitivity is
is $\mathcal{B}(\tau \to 3 \mu$) = 1.1  $\times 10^{-8}$.
...
signal (red) and background (blue) after all event selection cuts, in the event category 2, defined as containing at least one muon reconstructed by the new GEM  detectors in the first station of the upgraded muon system. The signal is shown for  $\mathcal{B}(\tau \to 3\mu) = 2\times10^{−8}$.}
